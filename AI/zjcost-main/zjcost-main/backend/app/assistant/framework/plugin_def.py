"""PluginDef — typed tool definitions with rich metadata.

Each tool carries:
- name, description, parameter schema
- read_only / destructive / concurrency_safe flags
- execute function with HandlerContext injection
...
Usage:
    ...
    def search_quotas(ctx: HandlerContext, *, keyword: str, top_n: int = 10) -> str:
        ...
"""
...
class PluginDef:
    ...
    # ── ModelCompat function calling schema ──

    def to_compatible_schema(self) -> dict[str, Any]:
        """Generate model-compatible function calling schema."""
    ...
    def execute(self, ctx: "HandlerContext", args: dict[str, Any]) -> str:
...
def _extract_params(func: Callable) -> list[ParamDef]:
    ...
    只跳过名为 ``self`` / ``cls`` / ``ctx`` 的参数。
    不再假设第一个非 self/cls 参数一定是 ctx —— 如果函数没有 ctx 参数，
    旧逻辑会静默跳过第一个真实参数，导致参数丢失。
    """
    ...
    for pname, p in sig.parameters.items():
        # 只跳过名为 self / cls / ctx 的参数（更安全，不依赖参数顺序）
        ...
        if pname in hints:
            py_type = hints[pname]
        elif has_default and p.default is not None:
...
def tool(
    *,
...
) -> Callable[[Callable[..., str]], PluginDef]:
    """Decorator to create a PluginDef from a function.
    ...
    The decorated name becomes a PluginDef instance (not the original function).
    """
    def decorator(func: Callable[..., str]) -> PluginDef:
...
def pre_hook(plugin_def: PluginDef) -> Callable:
    """Decorator to register a pre-execution hook on a PluginDef.
    ...
    def decorator(func: Callable) -> Callable:
        plugin_def.pre_hooks.append(func)
        return func
    return decorator


def post_hook(plugin_def: PluginDef) -> Callable:
    """Decorator to register a post-execution hook on a PluginDef.
    ...
    """
    def decorator(func: Callable) -> Callable:
        plugin_def.post_hooks.append(func)
        return func
    return decorator
