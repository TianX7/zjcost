"""ContextExtractor — auto-sediment conversational facts into Memory (Phase H7).

Closes the learning loop: after an handler run finishes, an extractor analyzes
(instruction, final answer) and proposes a small set of long-lived facts to
persist in the ContextStore.

## Design

- **ExtractedMemory**: lightweight DTO for a proposed memory entry.
- **ContextExtractor** ABC: `extract(instruction, answer, ctx) -> list[ExtractedMemory]`.
- **NoopContextExtractor**: default for agents that don't want extraction.
- **LLMContextExtractor**: uses the configured 辅助 provider to produce a
...
```python
class MyOrchestrator(BaseHandler):
    auto_save_memory = True
    context_extractor = LLMContextExtractor(max_items=3)

    def on_result(self, ctx, result):
        if self.auto_save_memory and ctx.memory is not None:
            for item in self.context_extractor.extract(
...
class NoopContextExtractor(ContextExtractor):
...
1. 只提取明显、可验证、未来有用的事实。例子：
   ...
   - 项目约定（"本项目按区域版 2018 定额计价"）→ scope=project
...
class LLMContextExtractor(ContextExtractor):
    """Extract memories by asking the configured 辅助 provider.

    Uses `generate_text` + strict JSON parsing. All failures degrade to
    empty list — extraction never breaks the main handler run.
    """
