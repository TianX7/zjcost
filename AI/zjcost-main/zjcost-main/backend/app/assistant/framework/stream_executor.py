...
Phase H2: inspired by standard StreamingToolExecutor pattern.

Key idea: in the synchronous pathway (`BaseHandler.run()`), we wait for the
...
from app.assistant.framework.types import HandlerStep, StepType
from app.assistant.providers.base import StreamEvent
...
class StreamRunResult:
    ...
    tool_steps: list[HandlerStep] = field(default_factory=list)
...
class StreamingToolExecutor:
    ...
    def _autofill_arguments(
        ...
        if tool_name in self._SEMANTIC_TOOLS_REQUIRING_QUERY:
            ...
            return arguments
        if tool_name == "load_extension":
            if not arguments.get("name") and instruction:
                top = self._resolve_top_extension_name(instruction)
                if top:
                    arguments["name"] = top
            return arguments
        # Generic fallback: any delegate_* sub-handler tool has a single required
        # `task` parameter. If the model omits it, inherit the current user
        # instruction so the sub-handler still receives actionable context
    ...
    def _execute_one(self, tc: dict[str, Any]) -> tuple[str, float]:
        # OPT-2: route through BaseHandler._execute_single_tool so the streaming
        ...
        try:
            result_str, duration_ms = BaseHandler._execute_single_tool(
