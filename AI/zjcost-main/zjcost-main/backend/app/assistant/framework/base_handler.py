"""BaseHandler — abstract base class with unified handler reasoning loop.

Eliminates the duplicated loop in valuation_handler, validation_handler, etc.
...
Usage:
    class ValuationHandler(BaseHandler):
        name = "valuation_handler"
        description = "辅助组价 Handler"
        system_prompt = "你是一位专业的工程计价辅助助手..."
        tool_names = ["search_quotas", "bind_quota", ...]

    handler = ValuationHandler()
    result = handler.run(ctx, instruction="为该清单项组价")
"""
...
from app.assistant.framework.model_switcher import model_switcher
...
class BaseHandler(ABC):
    """Abstract base class for all 辅助 agents.

    Subclasses define WHAT the handler does (prompt, tools).
    ...
    @abstractmethod
    def name(self) -> str:
        """Unique handler identifier (e.g. 'valuation_handler')."""
        ...
    ...
    def system_prompt(self) -> str:
        """System prompt for this handler."""
        ...

    @property
    @abstractmethod
    def tool_names(self) -> list[str]:
        """List of tool names this handler can use (from PluginRegistry)."""
        ...
    ...
    @property
    def tier(self) -> str:
        """模型层级："fast" / "balanced" / "powerful"。

        由 model_switcher 据此选择模型。子类可覆盖以指定默认层级。
        默认 "balanced"，保持向后兼容。
        """
        return "balanced"
    ...
    def max_turns(self) -> int:
        """Default max LLM round-trips. Override per handler if needed."""
        return 20
    ...
    def on_result(self, ctx: HandlerContext, result: HandlerResult) -> HandlerResult:
        """Post-processing hook. Override to add handler-specific fields to result."""
        return result
    ...
    def compact_threshold_tokens(self) -> int:
        ...
        Reasoning models (model-c etc.) have hidden reasoning tokens that
        multiply real cost, so a lower threshold is safer.
        """
        return 8000

    # ── Optional overrides (Phase H3: Handler Memory) ──
    ...
    def build_memory_context(self, ctx: HandlerContext) -> str:
        """Build a text block summarizing relevant memories for this run. (H3)

        Returns empty string if no context store is attached or no memories exist.
    ...
    def run(
        ...
        *,
        ...
        """Run the handler reasoning loop.
        ...
        Args:
            ...
            conversation_history: Prior turns as [{"role": "user"|"assistant", "content": str}, ...].
                ...
                message, so the handler sees the full dialogue. Memory/skill hints still apply
                only to the *new* instruction (via build_user_message).

        Returns:
            HandlerResult with answer, steps, and optional error.
        """
        ...
        trace = LogCollector(handler_name=self.name, ctx=ctx)
        trace.start(instruction)

        # Check 辅助 provider
        provider = get_zh_provider()
        if not provider.is_enabled() or not provider.is_configured():
            result = HandlerResult(answer="辅助 服务未配置，无法执行任务。", error="zh_not_configured")
            trace.finish(result)
            trace.persist()
            return result

        # 通过 model_switcher 选择模型（三级路由：fast/balanced/powerful）
        selected_model = model_switcher.route_model(self.name, instruction, tier=self.tier)
        trace.set_model_info(provider=type(provider).__name__, model=selected_model)
        ...
        tool_schemas = reg.get_compatible_schemas(effective_tool_names)
        if not tool_schemas:
            logger.warning("Handler '%s' has no tools resolved", self.name)
        ...
        while not effective_budget.should_force_answer:
            ...
            if over_token_budget or over_msg_budget:
                ...
                if bytes_saved > 0:
                    ...
                    logger.info(
                        "Handler '%s': microcompacted messages (saved %d bytes, tokens=%d, msgs=%d)",
            ...
            if over_token_budget and effective_budget.input_tokens_used > int(threshold * 1.5):
                ...
                logger.info(
                    "Handler '%s': full-compacted messages (tokens used: %d > %d)",
            ...
            # F5b: Fallback full compact by message count when API gives no usage.
                ...
                did_full_compact = True
                logger.info(
                    "Handler '%s': full-compacted messages (message count: %d > 20)",
            ...
            try:
                ...
                logger.error("Handler '%s' provider error at turn %d: %s", self.name, effective_budget.turns_used, exc)
                error_step = HandlerStep(type=StepType.ERROR, content=str(exc))
                ...
                if tool_steps and not answer_steps:
                    ...
                    result = self.on_result(ctx, HandlerResult(
            ...
            # A4: Log prompt-cache hits (auto-caching on some providers).
            cache_hit = turn_usage.get("cache_hit_tokens", 0)
            if cache_hit and turn_input > 0:
                logger.info(
                    "Handler '%s': prompt cache hit %d/%d tokens (%.0f%%)",
                    self.name, cache_hit, turn_input,
                    100 * cache_hit / turn_input,
                )

            # B2: Emit reasoning_content as a THINKING step for UI observability.
            # Do NOT append reasoning to ``messages`` — reasoning models explicitly
            # require stripping it on subsequent turns (the model re-derives).
            reasoning_text = turn_usage.get("reasoning_content")
            if reasoning_text:
                thinking_step = HandlerStep(
            ...
            if not response.get("tool_calls"):
                ...
                answer_step = HandlerStep(type=StepType.ANSWER, content=answer)
                ...
                result = self.on_result(ctx, HandlerResult(answer=answer, steps=steps))
                trace.finish(result)
                trace.persist()
                return result
            ...
            if response.get("content"):
                thinking_step = HandlerStep(type=StepType.THINKING, content=response["content"])
                steps.append(thinking_step)
                if on_step:
                    on_step(thinking_step)
            ...
            # ── Detect delegate sub-handler failures (anti-loop) ──
                        ...
                        if calls_made == 0 or has_error:
                            ...
                            if consecutive_delegate_failures[tool_name] >= 2:
                                warning = (
                                    ...
                                    f"请不要再调用同一个 Handler。换一种策略："
                                    f"使用其他 Handler，或你自己直接用工具完成任务。"
                                )
            ...
            if validation_feedback:
                feedback_step = HandlerStep(type=StepType.ERROR, content=validation_feedback)
            ...
            if (
                ...
                reflection_step = HandlerStep(type=StepType.THINKING, content="[系统注入反思节点]")
    ...
    def _execute_single_tool(
        ...
        plugin_def = reg.get(name)
        cache: dict[str, str] | None = None
        cache_key: str | None = None
        if plugin_def is not None and getattr(plugin_def, "read_only", False):
            try:
    ...
    def _execute_tools(
        ...
        ctx: HandlerContext,
        trace: LogCollector,
        on_step: Callable[[HandlerStep], None] | None,
    ) -> tuple[list[HandlerStep], list[dict[str, Any]]]:
        ...
        for tc in prepared_tool_calls:
            plugin_def = reg.get(tc["name"])
            safe = plugin_def.is_concurrency_safe if plugin_def else False
        ...
        for is_concurrent, batch in batches:
            ...
            if is_concurrent and len(batch) > 1:
                # ── Parallel execution ──
                logger.info(
                    "Handler '%s': running %d concurrency-safe tools in parallel",
                    self.name, len(batch),
                )
    ...
    def _prepare_tool_call(
        ...
        if tool_name in {"match_extensions_semantic", "search_memory_semantic"}:
            if not any(arguments.get(key) for key in ("query", "q", "task", "instruction")):
                if instruction:
                    arguments["query"] = instruction
        elif tool_name == "load_extension":
            if not arguments.get("name") and instruction:
                top = BaseHandler._resolve_top_extension_name(instruction)
                if top:
                    arguments["name"] = top
    ...
    @staticmethod
    def _resolve_top_extension_name(instruction: str) -> str | None:
        try:
            from app.assistant.framework.extension_registry import extension_registry
        except Exception:  # pragma: no cover — defensive
            return None
        try:
            scored = extension_registry.match_semantic(query=instruction, limit=1)
        except Exception:  # pragma: no cover — defensive
            return None
        if not scored:
            return None
                ...
                for key, val in list(parsed.items()):
                    if isinstance(val, list) and len(val) > 20:
                        ...
                        parsed[f"_{key}_truncated"] = (
                            ...
                            f"showing first 20 only (handler: {self.name})"
                        )
                zh = json.dumps(parsed, ensure_ascii=False)
                if len(zh) <= limit * 2:
                    return zh
        except (json.JSONDecodeError, TypeError):
            pass
    ...
    def _build_post_compact_snapshot(self, ctx: "HandlerContext") -> str:
    ...
    def streaming_enabled(self) -> bool:
        ...
        Override per handler to enable. Default: False for backward compatibility."""
        return False
    ...
    def stream_run(
        ...
        *,
        ...
        """Run the handler using the streaming path. (H2)
        ...
        if not provider.supports_streaming():
            logger.info(
                "Handler '%s': provider does not support streaming, using sync run()",
                self.name,
            )
            return self.run(ctx, instruction, on_step=on_step,
                            budget=budget, registry=registry,
        ...
        trace.set_model_info(
            provider=type(provider).__name__, model=selected_model
        )
        ...
        while not effective_budget.should_force_answer:
            ...
            if stream_cache_hit and in_tok > 0:
                logger.info(
                    "Handler '%s' (stream): prompt cache hit %d/%d tokens (%.0f%%)",
            ...
            if stream_result.error:
                error_step = HandlerStep(type=StepType.ERROR, content=stream_result.error)
                steps.append(error_step)
                if on_step:
                    on_step(error_step)
                result = self.on_result(ctx, HandlerResult(
                    answer=f"辅助 流式调用失败: {stream_result.error}",
                    steps=steps,
                    error="stream_error",
                ))
                trace.finish(result)
    ...
    def _microcompact_messages(
        ...
        - Does NOT remove any messages — keeps all assistant/tool_use + tool_result
          pairings intact so ``tool_call_id`` references stay valid (some providers
