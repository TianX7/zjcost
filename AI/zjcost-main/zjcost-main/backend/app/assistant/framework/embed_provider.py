"""EmbedProvider — abstraction for text-embedding backends (Phase H5).
...
- `HashEmbedProvider` — deterministic, pure-Python, no network. Used as
  the default/fallback; also ideal for unit tests where we need stable
  vectors without calling an external API.
- `ModelCompatEmbedProvider` — uses the `compatible` SDK (already in requirements)
  to call text-embedding-3-small / -large. Requires `MODEL_COMPAT_API_KEY`.
...
from app.assistant.providers.model_compat import ModelCompatHttpClient
...
# ───────────────────────────────────────────────────────────────────
# HashEmbedProvider — deterministic, offline, test-friendly
# ───────────────────────────────────────────────────────────────────
...
# ───────────────────────────────────────────────────────────────────
# ModelCompatEmbedProvider — production
# ───────────────────────────────────────────────────────────────────


class ModelCompatEmbedProvider(EmbedProvider):
    """Embedding via ModelCompat's `/v1/embeddings` endpoint.

    Reads `MODEL_COMPAT_API_KEY` + optional `MODEL_COMPAT_BASE_URL` from env.
    ...
    def __init__(
        ...
        *,
        ...
        key = api_key or os.environ.get("MODEL_COMPAT_API_KEY")
        if not key:
            raise EmbeddingError(
                "MODEL_COMPAT_API_KEY not set; cannot construct ModelCompatEmbedProvider"
            )
        # Read timeout from ZH_TIMEOUT_SECONDS (shared with LLM settings), default 120s.
        try:
            timeout_s = float(os.environ.get("ZH_TIMEOUT_SECONDS", "120"))
        except ValueError:
            timeout_s = 120.0
        self._client = ModelCompatHttpClient(
            api_key=key,
            base_url=base_url or os.environ.get("MODEL_COMPAT_BASE_URL"),
            timeout=timeout_s,
        )
        self._model = model
    ...
    @property
    def name(self) -> str:
        return f"compatible:{self._model}"
    ...
    def embed_many(self, texts: Iterable[str]) -> list[list[float]]:
        ...
        try:
            ...
            raise EmbeddingError(f"compatible embeddings call failed: {e}") from e

        vectors: list[list[float]] = []
        for item in resp.data:
            v = list(item.embedding)
            # Normalize defensively — ModelCompat already returns normalized
...
_active_provider: EmbedProvider | None = None


def get_embed_provider() -> EmbedProvider:
    """Return the active embedding provider.

    On first call, auto-detects: uses ModelCompat if `MODEL_COMPAT_API_KEY` is set
    ...
    if backend == "hash":
        _active_provider = HashEmbedProvider()
        logger.info("Embedding provider (forced): %s", _active_provider.name)
        return _active_provider

    if backend in ("compatible", ""):
        if os.environ.get("MODEL_COMPAT_API_KEY"):
            try:
                _active_provider = ModelCompatEmbedProvider()
                logger.info("Embedding provider: %s", _active_provider.name)
                return _active_provider
            except EmbeddingError as e:  # pragma: no cover
                logger.warning("ModelCompat embedding unavailable (%s); using hash fallback", e)
...
def set_embed_provider(provider: EmbedProvider | None) -> None:
...
def reset_embed_provider() -> None:
    """Reset the singleton so next get_embed_provider() re-initializes."""
