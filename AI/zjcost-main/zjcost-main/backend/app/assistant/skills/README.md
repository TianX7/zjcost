# Extensions — 领域知识模块（Phase H4）

Extensions 把专业领域知识（标准规范、合规要点、计价技巧）从 Handler 代码中解耦，
以可复用的 Markdown 模块形式组织。多个 Handler 可以共享同一个 Extension。
...
| 能执行工具吗 | 可以 | 不可以 |
| 谁加载 | Orchestrator / 用户 | Handler 声明或运行时 `load_skill` |
| 多处复用 | 每个 Handler 独立 | 多个 Handler 共享 |
...
description: 区域标准工程量计算方法第四版核心规则
triggers:
  - HKSMM4
  - 区域工程量
applies_to:
...
在 Handler YAML 配置中列出所用 Extension：
...
Handler 启动时会自动把 `hksmm4_basics.body` 拼接到 system prompt 之前。

### 2. 动态加载（运行时）

Handler 通过工具在对话中按需加载：

```python
# Handler decides mid-conversation:
...
- `hksmm4_basics` — HKSMM4 第四版基础规则（区域标准）
- `gb50500_compliance` — GB50500-2013 工程量清单计价规范合规要点（通用标准）
...
3. 重启应用（或在测试中手动调用 `extension_registry.register(parse_extension_file(...))`）。
4. 自动被 `list_skills` / `match_skills` 发现。

## 最佳实践

- **聚焦单一主题**：一个 Extension 一个领域，便于组合。
- **可执行的 checklist**：规则应可直接验证（"门窗洞口 > 0.5m² 扣减"）。
- **给示例和数字**：让推理服务能直接引用，避免空泛。
