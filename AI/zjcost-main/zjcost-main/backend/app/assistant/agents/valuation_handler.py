"""Valuation Handler — multi-step reasoning with tool calling for zh pricing.
...
The handler runs an iterative loop:
...
from app.assistant.providers import ZhProviderError, ToolCallRequest, get_zh_provider
...
# ════════════════════════════════════════════════════════════════
# Tool definitions (ModelCompat function calling schema)
# ════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════
# Handler step tracking
# ════════════════════════════════════════════════════════════════

@dataclass
class HandlerStep:
    """One step in the handler reasoning chain."""
...
@dataclass
class HandlerResult:
    """Complete result of an handler run."""
    answer: str
...
def _tool_calculate_cost(db: Session, boq: BoqItem, project_region: str) -> str:
    ...
    # _lookup_price 找不到价格时返回 None，统一降级为 0.0 以避免乘法异常
...
_SYSTEM_PROMPT = """\
你是一位专业的工程计价辅助助手，精通GB50500清单计价规范和定额组价方法。
...
def run_valuation_handler(
    *,
    ...
    """Run the valuation handler for a single BOQ item.
    ...
    Returns:
        HandlerResult with the final answer and all intermediate steps.
    """
    provider = get_zh_provider()
    if not provider.is_enabled() or not provider.is_configured():
        return HandlerResult(answer="辅助 服务未配置，无法执行辅助组价。", error="zh_not_configured")

    db: Session = next(get_db())
    try:
        return _run_handler_loop(
            provider=provider,
            db=db,
...
def _run_handler_loop(
    *,
    ...
    if not boq:
        return HandlerResult(answer="清单项不存在。", error="boq_not_found")
    ...
    for turn in range(MAX_AGENT_TURNS):
        try:
            response = provider.generate_with_tools(
                task="valuation_handler",
                messages=messages,
                tools=AGENT_TOOLS,
            )
        except ZhProviderError as exc:
            logger.error("Handler provider error at turn %d: %s", turn, exc)
        ...
        if not response["tool_calls"]:
            ...
            step = HandlerStep(type="answer", content=answer)
            steps.append(step)
            if on_step:
                on_step(step)
            return HandlerResult(
        ...
        if response["content"]:
            thinking_step = HandlerStep(type="thinking", content=response["content"])
