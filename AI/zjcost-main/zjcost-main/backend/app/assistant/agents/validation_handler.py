"""Validation Handler — 辅助-powered project data validation with tool calling.
...
# ════════════════════════════════════════════════════════════════
# Step tracking (shared structure with valuation_handler)
# ════════════════════════════════════════════════════════════════
...
_SYSTEM_PROMPT = """\
你是一位专业的工程造价审核辅助助手，精通GB50500工程量清单计价规范。
    ...
    *,
    ...
    if not provider.is_enabled() or not provider.is_configured():
        return ValidationHandlerResult(
            answer="辅助 服务未配置，无法执行辅助审核。", error="zh_not_configured"
        )
...
def _run_loop(
    *,
    ...
    if not project:
        return ValidationHandlerResult(answer="项目不存在。", error="project_not_found")
    ...
    if scope == "item" and boq_item_id:
        ...
        if not boq:
            return ValidationHandlerResult(answer="清单项不存在。", error="boq_not_found")
        user_msg = (
            f"请审核以下清单项:\n"
    ...
    for turn in range(MAX_AGENT_TURNS):
        try:
            response = provider.generate_with_tools(
                task="validation_handler",
                messages=messages,
                tools=AGENT_TOOLS,
            )
        except ZhProviderError as exc:
            logger.error("Validation handler error at turn %d: %s", turn, exc)
            return ValidationHandlerResult(
