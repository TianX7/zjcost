# Handler Configs — YAML-driven handler definitions (Phase H1)

This directory contains **declarative handler definitions**. Each `.md` file here
becomes a fully working handler at runtime, without writing any Python code.
...
tools:                     # list of tool names from PluginRegistry
  - search_quotas
  - get_quota_detail
---
...
2. Use only tool names that exist in the global `PluginRegistry`
   (see `app/assistant/tools/` for the full list of 22 tools).
3. Run the test suite — `test_handler_framework.py` validates all configs.
...
- No duplicate handler names across files
