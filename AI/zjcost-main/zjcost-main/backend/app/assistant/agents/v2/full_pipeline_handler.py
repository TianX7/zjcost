"""FullPipelineHandler — end-to-end automation from project setup to report.

Orchestrates a fixed sequence of stages:
1. ProjectSetupHandler  → create project + generate BOQ
2. CostExecuteHandler   → batch match quotas + bind
3. batch_calculate_project → compute costs
4. BatchReviewHandler   → quality check
...
class FullPipelineHandler(BaseHandler):
    ...
    def description(self) -> str:
        return "全流程自动化 Handler：从新建项目到生成计价报告的一键全自动流水线"
    ...
    def system_prompt(self) -> str:
        return """\
你是一个全流程自动化 Handler，负责一键完成从新建项目到生成计价报告的完整流程。

## 严格执行的五阶段流水线

每个阶段完成后用 **📌 阶段N完成** 标记，方便用户跟踪进度。

### 📌 阶段1：辅助开项（如果项目已有清单则跳过）
...
"""
    ...
    def build_user_message(self, ctx: HandlerContext, instruction: str) -> str:
        ...
        if ctx.project_id:
            ...
            # Inject current project state for zher stage-skipping
            try:
