"""CostPlanHandler — read-only pricing plan designer (Phase G2).
...
class CostPlanHandler(BaseHandler):
    """Read-only planning handler — designs pricing strategies without executing."""
    ...
    def description(self) -> str:
        return "组价方案设计 Handler：分析清单项特征，搜索候选定额，设计组价策略方案（不执行绑定）"
