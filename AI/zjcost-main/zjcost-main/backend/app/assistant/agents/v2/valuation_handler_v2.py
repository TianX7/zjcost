"""ValuationHandlerV2 — migrated to BaseHandler framework.

Replaces the 766-line valuation_handler.py with a ~80-line subclass.
All tool implementations now live in app.assistant.tools.quota_tools.
"""
...
class ValuationHandlerV2(BaseHandler):
    """Pricing handler — searches quotas, binds them, calculates costs."""

    @property
    def name(self) -> str:
        return "valuation_handler"

    @property
    def description(self) -> str:
        return "辅助组价 Handler：为清单项搜索定额、绑定定额、计算综合单价"
...
"""
    ...
    @staticmethod
    def _preview_unbound_items(ctx: "HandlerContext", limit: int = 25) -> str:
...
def run_valuation_handler_v2(
    *,
    ...
    """Drop-in replacement for the old run_valuation_handler().

    Returns HandlerResult (framework type) instead of the old dataclass.
    ...
    try:
        ...
        return handler.run(ctx, user_instruction, on_step=on_step)
    finally:
        db.close()
