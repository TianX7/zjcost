"""ValidationHandlerV2 — migrated to BaseHandler framework.

Replaces the 610-line validation_handler.py with a ~80-line subclass.
All tool implementations now live in app.assistant.tools.validation_tools.
"""
...
from app.assistant.framework.types import HandlerResult, StepType


class ValidationHandlerV2(BaseHandler):
    """Data validation handler — checks compliance, detects anomalies."""
    ...
    def description(self) -> str:
        return "数据审核 Handler：编码合规检查、消耗量异常检测、历史对比分析"
...
def run_validation_handler_v2(
    *,
    ...
    """Drop-in replacement for the old run_validation_handler()."""
    ...
    try:
        ...
        return handler.run(ctx, user_question, on_step=on_step)
    finally:
        db.close()
