"""CostExecuteHandler — full-permission execution handler (Phase G4).
...
class CostExecuteHandler(BaseHandler):
    """Full-permission execution handler — binds quotas, modifies prices, validates results."""
    ...
    def description(self) -> str:
        return "组价执行 Handler：执行定额绑定、单价修改、费用计算等写操作，并验证结果"
...
**你是执行 Handler，不是咨询 Handler。用户把任务委派给你，意味着已经决定要执行。你的工作是"干活"，不是"问话"。**
...
"""
    ...
    def build_user_message(self, ctx: HandlerContext, instruction: str) -> str:
        ...
        items so the handler has immediate data to act on, avoiding the
    ...
    @staticmethod
    def _preview_unbound_items(ctx: HandlerContext, limit: int = 30) -> str:
