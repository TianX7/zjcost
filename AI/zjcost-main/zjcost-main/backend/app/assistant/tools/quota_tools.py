...
from app.assistant.framework.plugin_def import tool
...
def _resolve_boq(ctx: HandlerContext, boq_item_id: int | None = None) -> BoqItem | None:
...
def get_quota_detail(ctx: HandlerContext, *, quota_item_id: int) -> str:
...
def list_current_bindings(ctx: HandlerContext, *, boq_item_id: int | None = None) -> str:
...
def calculate_cost(ctx: HandlerContext, *, boq_item_id: int | None = None) -> str:
...
def get_material_prices(ctx: HandlerContext) -> str:
    ...
    # 找不到价格时统一降级为 0.0，避免 JSON 中出现 null
    return json.dumps({
        "region": project_region,
        "labor_price": labor or 0.0,
        "material_price": material or 0.0,
        "machine_price": machine or 0.0,
    }, ensure_ascii=False)
...
def validate_binding(ctx: HandlerContext, *, boq_item_id: int | None = None) -> str:
...
def bind_quota(ctx: HandlerContext, *, boq_item_id: int | None = None, quota_item_id: int, coefficient: float = 1.0) -> str:
    boq = _resolve_boq(ctx, boq_item_id)
    if not boq:
        return json.dumps({"error": "未指定清单项，请传入 boq_item_id"}, ensure_ascii=False)
...
def unbind_quota(ctx: HandlerContext, *, boq_item_id: int | None = None, binding_id: int) -> str:
    boq = _resolve_boq(ctx, boq_item_id)
    if not boq:
        return json.dumps({"error": "未指定清单项，请传入 boq_item_id"}, ensure_ascii=False)
