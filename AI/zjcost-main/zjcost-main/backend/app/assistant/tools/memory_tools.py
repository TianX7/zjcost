"""Context tools — expose HandlerMemory operations to LLM agents (Phase H3).

These tools let agents persist and retrieve cross-session memories.
They resolve the ContextStore from HandlerContext.memory; if no store is
...
from app.assistant.framework.context_store import (
    ContextStore,
...
def _get_store(ctx: HandlerContext) -> ContextStore | None:
    return getattr(ctx, "memory", None)


def _resolve_scope_id(ctx: HandlerContext, scope: str, scope_id: int | None) -> int | None:
    if scope == "global":
        return None
...
@tool(
    ...
    description=(
        ...
        "发现确有值得长期保留的结论时才调用。"
        " 🚫 禁止在执行类请求（如辅助组价 / 批量绑定 / 处理未绑定项）的执行过程中调用，"
        " 这类请求应优先调用 delegate_valuation / delegate_execute 等写入型子 Handler。"
...
@tool(
    ...
    description=(
        "语义（embedding）检索记忆。比子串匹配更辅助，能找到语义相近的条目。"
