...
def get_project_summary_report(ctx: HandlerContext) -> str:
...
def generate_valuation_report(ctx: HandlerContext) -> str:
    project = ctx.db.query(Project).filter(Project.id == ctx.project_id).first()
    if not project:
        return json.dumps({"error": "项目不存在"}, ensure_ascii=False)
    ...
    # Token-efficient: truncate line_items for 辅助 context (keep full for first 30, summary for rest)
    MAX_DETAIL = 30
