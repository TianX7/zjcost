"""Base interfaces for pluggable 辅助 providers."""
...
class ZhProviderError(RuntimeError):
    """Generic provider execution error."""


class ZhProviderNotConfiguredError(ZhProviderError):
    """Raised when provider is disabled or missing required config."""


class ZhProviderNotAvailableError(ZhProviderError):
    """Raised when runtime dependency for provider is unavailable."""


class BaseZhProvider(ABC):
    @abstractmethod
    ...
    def generate_with_tools(
        ...
        *,
        ...
        """
        raise ZhProviderNotConfiguredError("Tool calling not supported")
...
class DisabledZhProvider(BaseZhProvider):
    def is_enabled(self) -> bool:
        return False
    ...
    def generate_structured(
        ...
        *,
        ...
        raise ZhProviderNotConfiguredError("辅助 provider is disabled")
