"""辅助 chat endpoint with project context injection."""
...
from app.assistant.agents.v2.chat_handler_v2 import chat_with_project_context_v2 as chat_with_project_context
from app.db.session import get_db
from app.models.boq_item import BoqItem
from app.models.calc_result import CalcResult
from app.models.line_item_quota_binding import LineItemQuotaBinding
from app.models.project import Project
from app.schemas.zh_insight import ZhChatRequest, ZhChatResponse
...
def _build_project_summary(project_id: int, db: Session) -> dict:
    """Build a compact project summary for the 辅助 context window."""
...
@router.post(
    "/projects/{project_id}/zh-chat",
    response_model=ZhChatResponse,
)
def zh_chat(
    project_id: int,
    payload: ZhChatRequest,
    db: Session = Depends(get_db),
) -> ZhChatResponse:
    """Chat with 辅助 assistant that has project context."""
    try:
        project_summary = _build_project_summary(project_id, db)
    except HTTPException:
        raise
    except Exception:
        # 优雅降级：summary 构建失败时记录日志
        logger.exception("zh_chat build_project_summary failed")
        return ZhChatResponse(reply=None, zh_available=False)
    ...
    try:
        ...
        logger.exception("zh_chat 错误")
        return ZhChatResponse(reply=None, zh_available=False)

    return ZhChatResponse(
        reply=reply,
        zh_available=reply is not None,
    )
