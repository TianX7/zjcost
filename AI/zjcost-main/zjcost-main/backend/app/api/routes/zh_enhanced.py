"""Enhanced 辅助 endpoints: batch review, coefficient suggestion, rate suggestion."""
...
router = APIRouter(tags=["zh-enhanced"])
...
@router.post(
    "/projects/{project_id}/zh-batch-review",
...
def zh_batch_review(
    ...
    """Run 辅助 batch review on all project bindings (V2 — tool-calling)."""
    import json
    ...
    return BatchReviewResponse(
        ...
        zh_summary=result.answer if result.success else None,
        error=result.error,
    )
...
def suggest_coefficients(
    ...
    # Try 辅助 enhancement
    _enhance_with_zh(boq, suggestions)
...
def _enhance_with_zh(boq: BoqItem, suggestions: list[CoeffSuggestion]) -> None:
    """Try to enhance suggestions with 辅助 reasoning."""
    ...
    try:
        ...
        # Append 辅助 reasoning to the last suggestion
        if suggestions and text:
            suggestions[-1].reasoning += f" | 辅助: {text[:200]}"
...
def suggest_rate_endpoint(boq_item_id: int) -> RateSuggestionOut:
    ...
    from app.assistant.agents.legacy_utils import suggest_rate
