"""辅助 Traces & Cost Dashboard API endpoints.

Provides:
- GET /assistant/traces — list recent handler traces with filtering
- GET /assistant/traces/stats — aggregated token usage and cost statistics
- GET /assistant/traces/{id} — single trace detail
"""
...
router = APIRouter(tags=["zh-traces"])
...
class CostStatsResponse(BaseModel):
    ...
    by_agent: list[HandlerStats]
    by_day: list[DayStats]


class HandlerStats(BaseModel):
    handler_name: str
    trace_count: int
    total_tokens: int
    total_cost_cents: float
...
@router.get("/assistant/traces", response_model=TraceListResponse)
def list_traces(
    ...
    """List recent handler traces with optional filters."""
    q = db.query(HandlerTrace)

    if project_id is not None:
        q = q.filter(HandlerTrace.project_id == project_id)
    if handler_name is not None:
        q = q.filter(HandlerTrace.handler_name == handler_name)
    if success is not None:
        q = q.filter(HandlerTrace.success == (1 if success else 0))

    total = q.count()
    rows = q.order_by(HandlerTrace.id.desc()).offset(offset).limit(limit).all()
...
@router.get("/assistant/traces/stats", response_model=CostStatsResponse)
def trace_stats(
    project_id: int | None = Query(None),
    ...
    q = db.query(HandlerTrace).filter(HandlerTrace.started_at >= cutoff)
    if project_id is not None:
        q = q.filter(HandlerTrace.project_id == project_id)
    ...
    # By handler
    handler_map: dict[str, list[HandlerTrace]] = {}
    for r in rows:
        handler_map.setdefault(r.handler_name, []).append(r)
    ...
    by_agent = [
        ...
        for name, traces in sorted(handler_map.items())
    ]

    # By day
    day_map: dict[str, list[HandlerTrace]] = {}
    for r in rows:
...
@router.get("/assistant/traces/{trace_id}", response_model=TraceDetailOut)
def get_trace_detail(
    trace_id: int,
    db: Session = Depends(get_db),
) -> TraceDetailOut:
    """Get detailed info for a single trace."""
    row = db.query(HandlerTrace).filter(HandlerTrace.id == trace_id).first()
    if not row:
...
def _to_trace_out(row: HandlerTrace) -> TraceOut:
    return TraceOut(
        id=row.id,
        project_id=row.project_id,
        handler_name=row.handler_name,
