...
def _sanitize_filename(name: str) -> str:
    """净化文件名中的特殊字符，避免污染 Content-Disposition 头。"""
    # 移除控制字符、路径分隔符和可能破坏响应头的引号
    cleaned = re.sub(r'[\\/<>|:"*?\x00-\x1f]', "_", name)
    return cleaned.strip().strip(".") or "project"
...
def get_report(
    ...
    # 避免除以 0：grand_total 为 0 时百分比显示 0
    grand = cost_summary.grand_total if cost_summary.grand_total else 0
    divisions = [
        DivisionRow(
            division=k,
            item_count=v["count"],
            ...
            percentage=f"{round(v['total'] / grand * 100, 1)}%" if grand else "0%",
        )
...
def export_report(
    ...
    if format == "excel":
        ...
        safe_name = _sanitize_filename(project.name)
        filename = f"valuation_report_{safe_name}_{project_id}.pdf"
        return StreamingResponse(
            io.BytesIO(file_bytes),
