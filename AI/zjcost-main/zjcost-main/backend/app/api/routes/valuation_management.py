...
def update_valuation_config(
    ...
    if payload.lock_standard and not config.locked_at:
        # locked_at 字段为 DateTime 类型，需传入 datetime 对象
        config.locked_at = datetime.now(timezone.utc)
    # updated_at 字段为 DateTime 类型，需传入 datetime 对象
    ...
    write_audit_log(
        ...
        after_json=json.dumps(
            ...
            ensure_ascii=False,
            default=str,  # locked_at 为 datetime 对象，需用 str 转换以支持 JSON 序列化
        ),
    )
...
def approve_contract_measurement(
    ...
    # approved_at 字段为 DateTime 类型，需传入 datetime 对象
    ...
    write_audit_log(
        ...
        after_json=json.dumps(
            ...
            ensure_ascii=False,
            default=str,  # approved_at 为 datetime 对象，需用 str 转换以支持 JSON 序列化
        ),
    )
    # audit_service 仅 flush 不 commit，路由层需显式提交以持久化审计日志
    db.commit()
...
def create_payment_certificate(
    ...
    # 校验 net_payable 非负，避免出现负数应付
    if net_payable < 0:
        raise HTTPException(
            status_code=400,
            detail="net_payable 不能为负数：gross_amount - prepayment_deduction - retention < 0",
        )
