...
def batch_confirm_bindings(
    ...
    db.commit()
    # 批量操作写入审计日志，记录受影响的 BOQ 项列表
    write_audit_log(
        db=db,
        project_id=next(iter(boq_map.values())).project_id if boq_map else 0,
        action="batch_confirm_quota_binding",
        resource_type="quota_binding",
        resource_id=0,
        after_json=json.dumps(
            {
                "boq_item_ids": list(boq_ids),
                "bindings_count": len(results),
            },
            ensure_ascii=False,
        ),
    )
