...
def _escape_like(value: str) -> str:
    """转义 LIKE 通配符 % 和 _，避免用户输入被当作通配符匹配。"""
...
def _log_audit(db: Session, project_id: int, action: str, before: dict | None = None, after: dict | None = None) -> None:
    db.add(AuditLog(
        ...
        # timestamp 字段为 DateTime 类型，需传入 datetime 对象
...
def _delete_project_graph(project: Project, db: Session) -> None:
    """Delete a project and all rows that are owned by it.

    SQLite foreign-key cascades are not guaranteed for existing local DB files,
    so project deletion is kept explicit to prevent orphan BOQ/calc rows.
    """
    boq_ids = [
        row[0]
        for row in db.query(BoqItem.id).filter(BoqItem.project_id == project.id).all()
    ]
    if boq_ids:
        db.query(CalcResult).filter(CalcResult.boq_item_id.in_(boq_ids)).delete(synchronize_session=False)
        db.query(LineItemQuotaBinding).filter(LineItemQuotaBinding.boq_item_id.in_(boq_ids)).delete(synchronize_session=False)
        db.query(BoqItem).filter(BoqItem.id.in_(boq_ids)).delete(synchronize_session=False)
    ...
    db.query(AuditLog).filter(AuditLog.project_id == project.id).delete(synchronize_session=False)
    db.delete(project)
...
def list_projects(
    ...
    if region:
        ...
        query = query.filter(Project.region.ilike(f"%{_escape_like(region)}%", escape="\\"))
    if q:
        pattern = f"%{_escape_like(q)}%"
        query = query.filter(or_(
            Project.name.ilike(pattern, escape="\\"),
            Project.description.ilike(pattern, escape="\\"),
            Project.region.ilike(pattern, escape="\\"),
...
def delete_project(project_id: int, db: Session = Depends(get_db)) -> dict:
    ...
    _delete_project_graph(project, db)
    db.commit()
...
class BatchIdsPayload(BaseModel):
    """批量操作请求体，限制单次最多 500 个 id，防止超大请求。"""
    ids: list[int] = Field(..., min_length=1, max_length=500)


@router.post(":batch-delete")
def batch_delete_projects(payload: BatchIdsPayload, db: Session = Depends(get_db)) -> dict:
    projects = db.query(Project).filter(Project.id.in_(payload.ids)).all()
    for project in projects:
        _delete_project_graph(project, db)
    db.commit()
    return {"ok": True, "deleted": len(projects)}


# ── Batch archive ──
@router.post(":batch-archive")
def batch_archive_projects(payload: BatchIdsPayload, db: Session = Depends(get_db)) -> dict:
    updated = (
        db.query(Project)
        .filter(Project.id.in_(payload.ids), Project.status != "archived")
...
def get_dashboard_summary(project_id: int, db: Session = Depends(get_db)) -> DashboardSummaryOut:
    ...
    boq_ids = [r.id for r in boq_items]
    boq_count = len(boq_items)
    dirty_count = sum(1 for r in boq_items if r.is_dirty)
    ...
    # Calc total + division breakdown from already-fetched boq_items
    ...
    if boq_ids:
        ...
        calc_total = sum(c.total_cost or 0 for c in calc_map.values())
...
def get_health_score(project_id: int, db: Session = Depends(get_db)) -> HealthScoreOut:
    ...
    if not has_desc:
        suggestions.append("添加项目描述有助于 辅助 辅助开项时理解需求")
