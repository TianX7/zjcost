...
def validate_project(project_id: int, db: Session) -> list[ValidationIssue]:
    ...
    required_price_categories = _required_price_categories(quota_by_id.values())
    for category in sorted(required_price_categories, key=lambda item: ("人工费", "材料费", "机械费").index(item)):
        if not _has_material_price(db, category=category, region=project_region):
            issues.append(ValidationIssue(
    ...
    for boq in boq_items:
        ...
        for b in bindings:
            ...
            if quota and normalize_unit(boq.unit) != normalize_unit(quota.unit):
                issues.append(ValidationIssue(
                    code="UNIT_MISMATCH",
                    severity=Severity.WARNING,
                    boq_item_id=boq.id,
            ...
            if quota and _quota_has_no_pricing_content(quota):
                issues.append(ValidationIssue(
    ...
    if std_codes:
        for boq in boq_items:
            ...
            if match:
                if normalize_unit(boq.unit) != normalize_unit(match.standard_unit):
                    issues.append(ValidationIssue(
    ...
    # 批量查询所有需要检查的 quota 的 resource detail 数量，避免 N+1 查询
    quota_ids_needing_check = [
        q.id for q in quota_by_id.values()
        if getattr(q, "has_resource_details", 0) == 1
    ]
    if quota_ids_needing_check:
        detail_counts = dict(
            db.query(QuotaResourceDetail.quota_item_id, func.count())
            .filter(QuotaResourceDetail.quota_item_id.in_(quota_ids_needing_check))
            .group_by(QuotaResourceDetail.quota_item_id)
            .all()
        )
    else:
        detail_counts = {}
    ...
    for boq in boq_items:
        ...
        for b in bindings:
            ...
            if quota and getattr(quota, "has_resource_details", 0) == 1:
                detail_count = detail_counts.get(quota.id, 0)
                if detail_count == 0:
                    issues.append(ValidationIssue(
...
def _has_material_price(db: Session, *, category: str, region: str) -> bool:
    return (
        db.query(MaterialPrice)
        .filter(
            MaterialPrice.name == category,
            or_(MaterialPrice.region == region, MaterialPrice.region == ""),
        )
        .first()
        is not None
    )
...
def _required_price_categories(quotas: Iterable[QuotaItem]) -> set[str]:
    """Return market price buckets actually required by bound quotas.

    Imported quota libraries may provide only a base price. Those can be priced
    without generic labor/material/machine market rates, so reporting missing
    material prices for them is noise rather than a useful validation issue.
    """
    categories: set[str] = set()
    for quota in quotas:
        if quota.labor_qty > 0:
            categories.add("人工费")
        if quota.material_qty > 0:
            categories.add("材料费")
        if quota.machine_qty > 0:
            categories.add("机械费")
    return categories


def _quota_has_no_pricing_content(quota: QuotaItem) -> bool:
    """True only when a quota has neither quantities, base price, nor details."""
    if quota.labor_qty != 0 or quota.material_qty != 0 or quota.machine_qty != 0:
        return False
    if (quota.base_price or 0) > 0:
        return False
    if getattr(quota, "has_resource_details", 0) == 1:
        return False
    return True
