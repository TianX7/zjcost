...
def create_snapshot(
    ...
    # 批量预加载所有 bindings 和 quotas，避免 N+1 查询
    boq_ids = [boq.id for boq, _ in line_results]
    all_bindings = (
    ...
    for b in all_bindings:
        bindings_by_boq.setdefault(b.boq_item_id, []).append(b)

    quota_ids = [b.quota_item_id for b in all_bindings]
    all_quotas = (
        db.query(QuotaItem).filter(QuotaItem.id.in_(quota_ids)).all()
        if quota_ids else []
    )
    quota_by_id = {q.id: q for q in all_quotas}
    ...
    snapshot_data = {
        ...
        "lines": line_data,
        "prices": {
            # _lookup_price 找不到价格时返回 None，快照中统一存 0.0 以便 diff 比较
            "labor": _lookup_price(db, category="人工费", region=region) or 0.0,
            "material": _lookup_price(db, category="材料费", region=region) or 0.0,
            "machine": _lookup_price(db, category="机械费", region=region) or 0.0,
        },
        "fee_config": {
    ...
    snap = Snapshot(
        ...
        label=label,
        # created_at 字段为 DateTime 类型，需传入 datetime 对象而非 isoformat 字符串
