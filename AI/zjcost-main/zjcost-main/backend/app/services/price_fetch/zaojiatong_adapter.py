"""Public concrete price adapter backed by 100njz market table."""
...
CONCRETE_URL = "https://www.100njz.com/concrete/"


class ZaojiatongAdapter(BasePriceFetcher):
    source_name = "zaojiatong"
    display_name = "百年建筑混凝土行情"

    _supported_categories = ["混凝土", "商品混凝土", "C30混凝土", "C35混凝土"]
    ...
    def _fetch_rows(self) -> list[dict[str, str | float]]:
        try:
            ...
            for row in _pick_market_rows(soup):
                ...
                mmdd = cells[3]
                rows.append({
                    "name": cells[0] or "商品混凝土",
                    "spec": cells[0] or "商品混凝土",
                    "unit": "m3",
                    "unit_price": price,
                    "region": cells[1],
            ...
            logger.warning("100njz concrete fetch failed: %s", exc)
            return []
