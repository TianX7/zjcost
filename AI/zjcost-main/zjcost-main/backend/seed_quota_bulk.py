...
from app.data.specialty_catalog import SPECIALTY_QUOTA_ITEMS
...
def gen_weak_current() -> list[tuple]:
    ...
    for i, (k, name, unit, l, m, mc) in enumerate(WEAK_CURRENT_ITEMS, start=1):
        code = f"D-P{i:03d}"
        items.append((code, name, unit, l, m, mc, "线缆敷设、设备安装、系统调试", f"适用{name}", "弱电辅助化"))
    return items
...
def gen_specialty() -> list[tuple]:
    ...
    for row in SPECIALTY_QUOTA_ITEMS:
        items.append((
            ...
            str(row["name"]),
            str(row["unit"]),
            float(row.get("labor_qty", 0)),
            float(row.get("material_qty", 0)),
            float(row.get("machine_qty", 0)),
            str(row.get("work_content", "")),
            str(row.get("applicable_scope", "")),
            str(row.get("chapter", "")),
        ))
    return items
...
def main():
    ...
    for row in all_items:
        ...
        dedup.append(row)
    if dup_count:
        print(f"WARN: 批次内重复 code 已去重: {dup_count} 条")
    ...
    try:
        ...
        for row in dedup:
            if len(row) == 10:
                code, discipline, name, unit, labor, mat, mach, work_content, applicable_scope, chapter = row
            else:
                code, name, unit, labor, mat, mach, work_content, applicable_scope, chapter = row
                discipline = "土建"
