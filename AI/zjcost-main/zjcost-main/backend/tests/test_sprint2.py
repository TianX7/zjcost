"""Sprint 2 integration tests: 辅助 matching, provenance, export, validation.
...
def _seed(db, project_id):
    ...
    # 注入材料价格数据，避免 _lookup_price 返回 None 导致 calc_total 为 0
    db.add_all([
        MaterialPrice(code="SP2-L", name="人工费", unit="工日", unit_price=100.0),
        MaterialPrice(code="SP2-M", name="材料费", unit="t", unit_price=50.0),
        MaterialPrice(code="SP2-N", name="机械费", unit="台班", unit_price=80.0),
    ])
    db.commit()
    db.refresh(boq1)
    db.refresh(boq2)
...
def test_quota_candidates(client, db):
    """辅助 match should return candidates ranked by relevance."""
