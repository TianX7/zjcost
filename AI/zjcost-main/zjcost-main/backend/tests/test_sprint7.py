...
def test_delete_project_removes_owned_boq_bindings_and_calc_results(client, db):
    r = client.post("/api/projects", json={"name": "DeleteGraph", "region": "bj"})
    pid = r.json()["id"]

    boq = BoqItem(project_id=pid, code="DEL-1", name="待删除清单", unit="m", quantity=2)
    quota = QuotaItem(quota_code="DEL-Q", name="待删除定额", unit="m", base_price=100, has_resource_details=0)
    db.add_all([boq, quota])
    db.flush()
    db.add(LineItemQuotaBinding(boq_item_id=boq.id, quota_item_id=quota.id))
    db.add(CalcResult(boq_item_id=boq.id, total_cost=200))
    boq_id = boq.id
    db.commit()
    ...
    assert db.query(LineItemQuotaBinding).filter(LineItemQuotaBinding.boq_item_id == boq_id).count() == 0
    assert db.query(CalcResult).filter(CalcResult.boq_item_id == boq_id).count() == 0
...
def test_calculate_supports_multi_quota_composition_with_coefficient(client, db):
    ...
    # 注入材料价格数据（人工费=1、材料费=1），使 direct_cost = (1*1 + 1*1) * 10 = 20
    db.add_all([
        MaterialPrice(code="MQ-L", name="人工费", unit="工日", unit_price=1, region="bj", effective_date="2025-01-01"),
        MaterialPrice(code="MQ-M", name="材料费", unit="t", unit_price=1, region="bj", effective_date="2025-01-01"),
        MaterialPrice(code="MQ-N", name="机械费", unit="台班", unit_price=1, region="bj", effective_date="2025-01-01"),
    ])
    db.commit()
...
def test_provenance_contains_unit_price_and_breakdown(client, db):
    ...
    # 注入材料价格数据，使 labor_price > 0 以满足断言
    db.add_all([
        MaterialPrice(code="PB-L", name="人工费", unit="工日", unit_price=100, region="bj", effective_date="2025-01-01"),
        MaterialPrice(code="PB-M", name="材料费", unit="t", unit_price=100, region="bj", effective_date="2025-01-01"),
        MaterialPrice(code="PB-N", name="机械费", unit="台班", unit_price=100, region="bj", effective_date="2025-01-01"),
    ])
    db.commit()
