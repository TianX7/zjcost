"""Integration tests for the Handler Framework (Phase A-E).

Tests cover:
- T1: PluginDef — schema generation, parameter extraction, execution
- T2: PluginRegistry — register, lookup, execute, concurrency partition
- T3: TokenBudget — limits and tracking
- T4: LogCollector — data collection and cost estimation
- T5: ModelSwitcher — routing logic and complexity heuristic
- T6: Pipeline — sequential execution, error handling, skip
- T7: handler_to_tool — Handler-as-Tool wrapping
- T8: BaseHandler — loop, trace integration, budget exceeded
"""
...
from app.assistant.framework.plugin_def import ParamDef, PluginDef, tool, _extract_params
from app.assistant.framework.plugin_registry import PluginRegistry
from app.assistant.framework.log_collector import LogCollector, estimate_cost_cents
from app.assistant.framework.model_switcher import (
...
def _mock_ctx(**kwargs) -> HandlerContext:
    """Create a lightweight mock HandlerContext (no real DB)."""
    return HandlerContext(db=MagicMock(), project_id=kwargs.get("project_id", 1), **{
...
class TestPluginDef:
    """Tests for PluginDef schema generation, param extraction, and execution."""

    def test_to_compatible_schema_basic(self):
        td = PluginDef(
            name="my_tool",
        ...
        schema = td.to_compatible_schema()
    ...
    def test_to_compatible_schema_no_required(self):
        td = PluginDef(
            name="t",
            description="d",
    ...
    def test_execute_exception_returns_error_json(self):
        ...
        td = PluginDef(name="t", description="d", func=failing_func)
    ...
    def test_concurrency_safe_defaults_to_read_only(self):
        assert PluginDef(name="t", description="d", read_only=True).is_concurrency_safe is True
        assert PluginDef(name="t", description="d", read_only=False).is_concurrency_safe is False

    def test_concurrency_safe_explicit_override(self):
        td = PluginDef(name="t", description="d", read_only=False, concurrency_safe=True)
...
class TestPluginRegistry:
    """Tests for PluginRegistry registration, lookup, and execution."""

    def _make_registry(self) -> PluginRegistry:
        reg = PluginRegistry()
        reg.register(PluginDef(
            name="tool_a",
            description="Tool A",
    ...
    def test_get_compatible_schemas(self):
        reg = self._make_registry()
        schemas = reg.get_compatible_schemas(["tool_a"])
    ...
    def test_register_many(self):
        reg = PluginRegistry()
        t1 = PluginDef(name="x", description="x")
        t2 = PluginDef(name="y", description="y")
        reg.register_many(t1, t2)
        assert len(reg) == 2

    def test_overwrite_existing(self):
        reg = PluginRegistry()
        reg.register(PluginDef(name="x", description="old"))
        reg.register(PluginDef(name="x", description="new"))
        assert reg.get("x").description == "new"

    def test_partition_by_concurrency(self):
        reg = PluginRegistry()
        reg.register(PluginDef(name="r1", description="", read_only=True))
        reg.register(PluginDef(name="r2", description="", read_only=True))
        reg.register(PluginDef(name="w1", description="", read_only=False))
        reg.register(PluginDef(name="r3", description="", read_only=True))
...
class TestLogCollector:
    """Tests for LogCollector data collection and cost estimation."""

    def test_basic_collection(self):
        ctx = _mock_ctx()
        tc = LogCollector(handler_name="test_agent", ctx=ctx)
    ...
    def test_finish_and_summary(self):
        ctx = _mock_ctx()
        tc = LogCollector(handler_name="test", ctx=ctx)
        tc.start("test")
        tc.set_model_info(provider="TestProvider", model="model-a")
        tc.record_turn(500, 200)

        result = HandlerResult(answer="done", steps=[])
        tc.finish(result)

        s = tc.summary()
        assert s["handler"] == "test"
    ...
    def test_persist_no_db_returns_none(self):
        ctx = HandlerContext(db=None, project_id=1)
        tc = LogCollector(handler_name="test", ctx=ctx)
        tc.start("test")
        tc.finish(HandlerResult(answer="ok"))
        assert tc.persist() is None

    def test_cost_estimation_known_model(self):
        # model-a: input=$2.50/M, output=$10.00/M
        cost = estimate_cost_cents("model-a", 1_000_000, 0)
        assert cost == 250.0  # $2.50 = 250 cents

    def test_cost_estimation_output_heavy(self):
        # model-a: output=$10.00/M → 500k output = $5 = 500 cents
        cost = estimate_cost_cents("model-a", 0, 500_000)
        assert cost == 500.0
    ...
    def test_cost_estimation_zero_tokens(self):
        assert estimate_cost_cents("model-a", 0, 0) == 0.0
...
class TestModelSwitcher:
    ...
    def test_known_handler_default_tier(self):
        assert route_model("query_handler").level == 1  # fast
        assert route_model("valuation_handler").level == 2  # balanced
        assert route_model("orchestrator").level == 3  # powerful

    def test_unknown_handler_gets_balanced(self):
    ...
    def test_force_tier_override(self):
        ...
        assert route_model("query_handler", force_tier=3).level == 3

    def test_complexity_bumps_tier_up(self):
        # query_handler defaults to fast, but complex instruction bumps to balanced
        tier = route_model("query_handler", "请全面分析并详细对比各分部的造价结构，给出优化策略方案")
        assert tier.level >= 2
...
class TestPipeline:
    ...
    def test_sequential_execution(self):
        ...
        handler_a = self._make_mock_agent("a", "result_a")
        handler_b = self._make_mock_agent("b", "result_b")

        pipeline = Pipeline("test", [
            Stage(agent=handler_a, instruction="do A"),
            Stage(agent=handler_b, instruction="do B"),
        ])
        ...
        assert result.stages[0].handler_name == "a"
        assert result.stages[1].handler_name == "b"

    def test_stop_on_error(self):
        from app.assistant.framework.pipeline import Pipeline, Stage

        handler_a = self._make_mock_agent("a", "ok")
        handler_fail = self._make_mock_agent("fail", "failed", success=False)
        handler_c = self._make_mock_agent("c", "should not run")

        pipeline = Pipeline("test", [
            Stage(agent=handler_a, instruction="A"),
            Stage(agent=handler_fail, instruction="FAIL"),
            Stage(agent=handler_c, instruction="C"),
        ], stop_on_error=True)
        result = pipeline.run(_mock_ctx())

        assert not result.success
        assert result.stage_count == 2  # C should not run
        assert "fail" in result.stages[1].handler_name

    def test_continue_on_error(self):
        from app.assistant.framework.pipeline import Pipeline, Stage

        handler_fail = self._make_mock_agent("fail", "failed", success=False)
        handler_ok = self._make_mock_agent("ok", "recovered")

        pipeline = Pipeline("test", [
            Stage(agent=handler_fail, instruction="FAIL"),
            Stage(agent=handler_ok, instruction="OK"),
        ], stop_on_error=False)
        result = pipeline.run(_mock_ctx())
    ...
    def test_skip_stage(self):
        ...
        handler_a = self._make_mock_agent("a", "result_a")
        handler_skip = self._make_mock_agent("skip", "should not run")

        pipeline = Pipeline("test", [
            Stage(agent=handler_a, instruction="A"),
            Stage(agent=handler_skip, instruction="SKIP",
    ...
    def test_on_stage_callback(self):
        ...
        handler = self._make_mock_agent("a", "done")
        events = []

        pipeline = Pipeline("test", [Stage(agent=handler, instruction="A")])
    ...
    def test_empty_pipeline(self):
        from app.assistant.framework.pipeline import Pipeline
    ...
    def test_exception_in_stage(self):
        ...
        pipeline = Pipeline("test", [
            Stage(agent=CrashHandler(), instruction="X"),
        ], stop_on_error=True)
        result = pipeline.run(_mock_ctx())
...
class TestHandlerAsTool:
    """Tests for handler_to_tool wrapping."""
    ...
    def _make_mock_agent(self):
        ...
        class SimpleHandler(BaseHandler):
            ...
            def tool_names(self_inner):
                return []
            def run(self_inner, ctx, instruction, *, on_step=None, budget=None, registry=None):
                return HandlerResult(answer=f"Processed: {instruction}")
    ...
    def test_wraps_to_tooldef(self):
        ...
        handler = self._make_mock_agent()
        td = handler_to_tool(agent)
        assert isinstance(td, PluginDef)
    ...
    def test_custom_name_override(self):
        ...
        handler = self._make_mock_agent()
        td = handler_to_tool(handler, name_override="custom_name")
...
# ═════════════════════════════════════════════════════════════════
# T8: BaseHandler (with mocked LLM)
# ═════════════════════════════════════════════════════════════════

class TestBaseHandler:
    """Tests for BaseHandler loop, trace integration, budget handling."""
    ...
    def test_disabled_provider_returns_error(self):
        HandlerClass = self._make_handler_class()
        handler = HandlerClass()

        with patch("app.assistant.framework.base_handler.get_zh_provider", return_value=self._make_disabled_provider()):
            result = handler.run(_mock_ctx(), "test")

        assert not result.success
        assert result.error == "zh_not_configured"

    def test_simple_answer_no_tools(self):
        HandlerClass = self._make_handler_class()
        handler = HandlerClass()

        with patch("app.assistant.framework.base_handler.get_zh_provider", return_value=self._make_simple_provider("Hello!")):
            result = handler.run(_mock_ctx(), "say hello")
    ...
    def test_tool_call_loop(self):
        ...
        with patch("app.assistant.framework.base_handler.get_zh_provider", return_value=provider):
            result = handler.run(_mock_ctx(), "search", registry=reg)
    ...
    def test_validation_error_is_fed_back_and_handler_can_recover(self):
        HandlerClass = self._make_handler_class()
        handler = HandlerClass()
    ...
    def test_repeated_validation_error_aborts_handler_loop(self):
        HandlerClass = self._make_handler_class()
        handler = HandlerClass()
    ...
    def test_budget_exceeded(self):
        ...
        reg = PluginRegistry()
        reg.register(PluginDef(name="mock_tool", description="m", func=lambda ctx, **kw: "{}"))

        budget = TokenBudget(max_turns=2)

        with patch("app.assistant.framework.base_handler.get_zh_provider", return_value=provider):
            result = handler.run(_mock_ctx(), "loop", budget=budget, registry=reg)
    ...
    def test_on_step_callback(self):
        ...
        with patch("app.assistant.framework.base_handler.get_zh_provider", return_value=self._make_simple_provider("OK")):
            handler.run(_mock_ctx(), "test", on_step=lambda s: steps_received.append(s))

        assert len(steps_received) > 0
        assert steps_received[-1].type == StepType.ANSWER

    def test_provider_error_traced(self):
        from app.assistant.providers import ZhProviderError
        ...
        provider.is_enabled.return_value = True
        provider.is_configured.return_value = True
        provider.generate_with_tools.side_effect = ZhProviderError("api down")
...
class TestGlobalRegistryIntegrity:
    """Verify all tools are registered and all handlers reference valid tools."""
    ...
    def test_all_handler_tool_references_valid(self):
        import app.assistant.tools
        from app.assistant.framework.plugin_registry import registry
        from app.assistant.agents.v2 import (
            ValuationHandlerV2, ValidationHandlerV2, ChatHandlerV2,
            BoqHandlerV2, QueryHandlerV2, InsightHandlerV2,
            QuotaMatchHandlerV2, BatchReviewHandlerV2, RateSuggestionHandlerV2,
        )
        ...
        for handler in agents:
            ...
            assert missing == set(), f"Handler '{handler.name}' references missing tools: {missing}"
    ...
    def test_orchestrator_stream_run_uses_handler_registry(self):
        ...
        def fake_super_stream_run(self, ctx, instruction, *, on_step=None, budget=None, registry=None, conversation_history=None):
            ...
            return HandlerResult(answer="ok")

        with patch("app.assistant.framework.base_handler.BaseHandler.stream_run", fake_super_stream_run):
            orch.stream_run(_mock_ctx(), "辅助组价")

        assert captured["registry"] is orch._handler_registry
        assert captured["registry"].get("delegate_execute") is not None

    def test_pipelines_build(self):
        from app.assistant.pipelines import build_pricing_pipeline, build_audit_pipeline
...
class TestHookSystem:
    ...
    def test_pre_hook_continue(self):
        from app.assistant.framework.plugin_def import HookAction, HookResult
    ...
    def test_post_hook_appends_note(self):
        from app.assistant.framework.plugin_def import HookResult
    ...
    def test_decorator_registration(self):
        from app.assistant.framework.plugin_def import pre_hook, post_hook

        td = PluginDef(name="t", description="d", func=lambda ctx, **kw: "{}")
...
# ═════════════════════════════════════════════════════════════════
# T11: Tool Concurrency in BaseHandler (F1)
# ═════════════════════════════════════════════════════════════════

class TestToolConcurrency:
    """Tests for concurrent tool execution in BaseHandler._execute_tools (F1)."""

    def _make_handler_class(self):
        from app.assistant.framework.base_handler import BaseHandler

        class ConcHandler(BaseHandler):
            @property
            def name(self_inner):
                return "conc_agent"
            @property
            def description(self_inner):
                return "Concurrency test handler"
            @property
            def system_prompt(self_inner):
                return ""
            @property
    ...
    def test_parallel_batch_runs_concurrently(self):
        ...
        reg = PluginRegistry()
        reg.register(PluginDef(name="read_a", description="", func=read_func, read_only=True))
        reg.register(PluginDef(name="read_b", description="", func=read_func, read_only=True))
        ...
        trace = MagicMock()
        steps, msgs = handler._execute_tools(tool_calls, reg, _mock_ctx(), trace, None)
    ...
    def test_serial_for_non_safe_tools(self):
        ...
        reg = PluginRegistry()
        reg.register(PluginDef(name="write_c", description="", func=write_func, read_only=False))
    ...
    def test_mixed_partitioning(self):
        ...
        reg = PluginRegistry()
        reg.register(PluginDef(name="read_a", description="", func=generic_func, read_only=True))
        reg.register(PluginDef(name="read_b", description="", func=generic_func, read_only=True))
        reg.register(PluginDef(name="write_c", description="", func=generic_func, read_only=False))
    ...
    def test_parallel_error_handling(self):
        ...
        reg = PluginRegistry()
        reg.register(PluginDef(name="read_a", description="", func=ok_func, read_only=True))
        reg.register(PluginDef(name="read_b", description="", func=fail_func, read_only=True))
    ...
    def test_on_step_callback_fired(self):
        ...
        reg = PluginRegistry()
        reg.register(PluginDef(name="read_a", description="", func=lambda ctx, **kw: "{}", read_only=True))

        tool_calls = [{"id": "1", "name": "read_a", "arguments": {}}]
        received = []
        trace = MagicMock()
        handler._execute_tools(tool_calls, reg, _mock_ctx(), trace, lambda s: received.append(s))
    ...
    def test_prepare_tool_call_autofills_query_for_semantic_tools(self):
        ...
        ctx = _mock_ctx()
        ctx.metadata["current_instruction"] = "示例区工程量计量规则"
        tool_calls = [{"id": "1", "name": "match_skills_semantic", "arguments": {}}]
        trace = MagicMock()

        steps, msgs = handler._execute_tools(tool_calls, reg, ctx, trace, None)

        assert steps[0].tool_args == {"query": "示例区工程量计量规则"}
        assert json.loads(msgs[0]["content"]) == {"query": "示例区工程量计量规则"}


# ═════════════════════════════════════════════════════════════════
# T12: Read-Only Handler Mode (F2)
# ═════════════════════════════════════════════════════════════════
...
class TestReadOnlyMode:
    ...
    def test_read_only_filters_destructive(self):
        ...
        reg = PluginRegistry()
        reg.register(PluginDef(name="safe_read", description="", read_only=True, destructive=False,
                             func=lambda ctx, **kw: "{}"))
        reg.register(PluginDef(name="danger_write", description="", destructive=True,
                             func=lambda ctx, **kw: "{}"))
        ...
        with patch("app.assistant.framework.base_handler.get_zh_provider", return_value=provider):
            handler.run(_mock_ctx(), "test", registry=reg)
...
class TestAutoCompaction:
    ...
    def test_no_compact_when_short(self):
        handler = self._make_agent()
        msgs = [
            {"role": "system", "content": "sys"},
            {"role": "user", "content": "hello"},
            {"role": "assistant", "content": "hi"},
        ]
        result = handler._compact_messages(msgs)
    ...
    def test_sanitize_messages_drops_orphan_tool_messages(self):
        ...
        result = handler._sanitize_messages_for_provider(msgs)
...
class TestDomainContext:
    """Tests for HandlerContext.build_project_context (F4)."""

    def test_returns_empty_when_no_project(self):
        ctx = HandlerContext(db=MagicMock(), project_id=999)
    ...
    def test_returns_cached_summary(self):
        ctx = HandlerContext(db=MagicMock(), project_id=1)
...
# ═════════════════════════════════════════════════════════════════
# T15: CostExploreHandler (G1)
# ═════════════════════════════════════════════════════════════════

class TestCostExploreHandler:
    """Tests for CostExploreHandler (Phase G1)."""

    def test_is_read_only(self):
        from app.assistant.agents.v2.cost_explore_handler import CostExploreHandler
        handler = CostExploreHandler()
        assert handler.read_only is True
        assert handler.name == "cost_explore"
    ...
    def test_has_only_read_tools(self):
        ...
        for tool_name in handler.tool_names:
            ...
            assert not td.destructive, f"Explore handler has destructive tool: {tool_name}"

    def test_max_turns_is_low(self):
        from app.assistant.agents.v2.cost_explore_handler import CostExploreHandler
        assert CostExploreHandler().max_turns <= 8

    def test_high_concurrency(self):
        from app.assistant.agents.v2.cost_explore_handler import CostExploreHandler
        assert CostExploreHandler().max_tool_concurrency >= 3


# ═════════════════════════════════════════════════════════════════
# T16: CostPlanHandler (G2)
# ═════════════════════════════════════════════════════════════════

class TestCostPlanHandler:
    """Tests for CostPlanHandler (Phase G2)."""

    def test_is_read_only(self):
        from app.assistant.agents.v2.cost_plan_handler import CostPlanHandler
        handler = CostPlanHandler()
        assert handler.read_only is True
        assert handler.name == "cost_plan"
    ...
    def test_has_no_write_tools(self):
        ...
        handler_tools = set(agent.tool_names)
        assert handler_tools.isdisjoint(write_tools), \
            f"Pla handler should not have write tools: {handler_tools & write_tools}"
    ...
    def test_build_user_message_with_boq(self):
        ...
        ctx.resolve_region = lambda: "示例省"

        handler = CostPlanHandler()
        msg = handler.build_user_message(ctx, "")
...
# ═════════════════════════════════════════════════════════════════
# T17: CostValidationHandler (G3)
# ═════════════════════════════════════════════════════════════════

class TestCostValidationHandler:
    """Tests for CostValidationHandler (Phase G3)."""

    def test_is_read_only(self):
        from app.assistant.agents.v2.cost_validation_handler import CostValidationHandler
        handler = CostValidationHandler()
        assert handler.read_only is True
        assert handler.name == "cost_validation"
    ...
    def test_compact_threshold_enabled(self):
        from app.assistant.agents.v2.cost_validation_handler import CostValidationHandler
        assert CostValidationHandler().compact_threshold_tokens > 0

    def test_on_result_extracts_issues(self):
        from app.assistant.agents.v2.cost_validation_handler import CostValidationHandler

        handler = CostValidationHandler()
        result = HandlerResult(answer="found issues", steps=[
            HandlerStep(
        ...
        result = handler.on_result(_mock_ctx(), result)
...
# ═════════════════════════════════════════════════════════════════
# T18: CostExecuteHandler (G4)
# ═════════════════════════════════════════════════════════════════

class TestCostExecuteHandler:
    """Tests for CostExecuteHandler (Phase G4)."""

    def test_is_not_read_only(self):
        from app.assistant.agents.v2.cost_execute_handler import CostExecuteHandler
        handler = CostExecuteHandler()
        assert handler.read_only is False
        assert handler.name == "cost_execute"
    ...
    def test_has_validation_tools(self):
        from app.assistant.agents.v2.cost_execute_handler import CostExecuteHandler
        handler = CostExecuteHandler()
        assert "validate_binding" in handler.tool_names
        assert "calculate_cost" in handler.tool_names
...
# ═════════════════════════════════════════════════════════════════
# T19: ModelSwitcher Phase G Tiers
# ═════════════════════════════════════════════════════════════════

class TestModelSwitcherPhaseG:
    """Tests for Phase G handler tier assignments."""
...
# ═════════════════════════════════════════════════════════════════
# T20: Phase G Handler Tool References Validity
# ═════════════════════════════════════════════════════════════════

class TestPhaseGToolReferences:
    """Verify all Phase G agents reference valid tools from the global registry."""

    def test_all_phase_g_handler_tools_exist(self):
        import app.assistant.tools
        from app.assistant.framework.plugin_registry import registry
        from app.assistant.agents.v2 import (
            CostExploreHandler, CostPlanHandler, CostValidationHandler, CostExecuteHandler,
        )

        all_tools = set(registry.all_names)
        agents = [
            CostExploreHandler(), CostPlanHandler(),
            CostValidationHandler(), CostExecuteHandler(),
        ]
        for handler in agents:
            missing = set(agent.tool_names) - all_tools
            assert missing == set(), f"Phase G handler '{handler.name}' references missing tools: {missing}"


# ═════════════════════════════════════════════════════════════════
# T21: HandlerDefinition parsing (H1)
# ═════════════════════════════════════════════════════════════════

class TestHandlerDefinition:
    """Tests for HandlerDefinition parsing from YAML frontmatter."""

    def test_parse_minimal_valid(self):
        from app.assistant.framework.handler_definition import parse_handler_text
...
You are a test handler."""
        defn = parse_handler_text(text)
        assert defn.name == "my_agent"
        assert defn.description == "样例"
        assert defn.tool_names == ["search_boq"]
        assert defn.system_prompt == "You are a test handler."
...
System prompt body here."""
    ...
    def test_missing_frontmatter_raises(self):
        from app.assistant.framework.handler_definition import (
            HandlerDefinitionError, parse_handler_text,
        )

        with pytest.raises(HandlerDefinitionError, match="missing YAML frontmatter"):
            parse_handler_text("no frontmatter here\n\njust body")
...
body"""
        with pytest.raises(HandlerDefinitionError, match="invalid YAML"):
            parse_handler_text(text)
...
body"""
        with pytest.raises(HandlerDefinitionError, match="snake_case"):
            parse_handler_text(text)
...
"""
        with pytest.raises(HandlerDefinitionError, match="system_prompt"):
            parse_handler_text(text)
...
body"""
        with pytest.raises(HandlerDefinitionError, match="fast"):
            parse_handler_text(text)
...
body"""
        with pytest.raises(HandlerDefinitionError, match="tools"):
            parse_handler_text(text)
...
file body""", encoding="utf-8")
        ...
        defn = parse_handler_file(f)
        assert defn.name == "file_agent"
        assert defn.source_file == str(f)


# ═════════════════════════════════════════════════════════════════
# T22: ConfigurableHandler (H1)
# ═════════════════════════════════════════════════════════════════

class TestConfigurableHandler:
    """Tests for ConfigurableHandler driven by HandlerDefinition."""
    ...
    def _make_def(self, **kwargs):
        ...
        return HandlerDefinition(**defaults)
    ...
    def test_properties_mirror_definition(self):
        ...
        handler = ConfigurableHandler(defn)
        assert handler.name == "abc"
        assert handler.description == "desc"
        assert handler.tool_names == ["a", "b"]
        assert handler.max_turns == 7
        assert handler.read_only is True
        assert handler.max_tool_concurrency == 3
        assert handler.compact_threshold_tokens == 1000
        assert handler.model_tier == "fast"

    def test_tool_names_is_copy(self):
        """Mutating handler.tool_names should not affect the definition."""
        from app.assistant.framework.configurable_handler import ConfigurableHandler
        defn = self._make_def(tool_names=["a", "b"])
        handler = ConfigurableHandler(defn)
        handler.tool_names.append("c")
...
# ═════════════════════════════════════════════════════════════════
# T23: handler_loader directory scanning (H1)
# ═════════════════════════════════════════════════════════════════

class TestHandlerLoader:
    ...
    def test_load_definitions_from_dir(self, tmp_path):
        from app.assistant.framework.handler_loader import load_definitions_from_dir
...
body""")
    ...
    def test_strict_raises_on_invalid(self, tmp_path):
        from app.assistant.framework.handler_definition import HandlerDefinitionError
        from app.assistant.framework.handler_loader import load_definitions_from_dir

        self._write(tmp_path, "bad.md", "no frontmatter")
        with pytest.raises(HandlerDefinitionError):
...
---
body""")
        with pytest.raises(HandlerDefinitionError, match="duplicate"):
            load_definitions_from_dir(tmp_path, strict=True)
    ...
    def test_validate_tool_references(self, tmp_path):
        import app.assistant.tools  # noqa: F401 — register tools
        from app.assistant.framework.handler_definition import HandlerDefinition
        from app.assistant.framework.handler_loader import validate_tool_references

        defn_good = HandlerDefinition(
            name="x", description="x", system_prompt="x",
            tool_names=["search_boq"],
        )
        assert validate_tool_references(defn_good) == []

        defn_bad = HandlerDefinition(
            name="x", description="x", system_prompt="x",
...
---
body""")
        with pytest.raises(HandlerDefinitionError, match="unknown tools"):
...
class TestExampleConfigs:
    ...
    def test_load_shipped_configs(self):
        ...
        configs_dir = Path(__file__).parent.parent / "app" / "zh" / "agents" / "configs"
...
def _make_stream_tool(name: str, result: str = '{"ok":1}',
    ...
    """Helper: build a PluginDef returning a canned result."""
...
class TestStreamingToolExecutor:
    ...
    def test_content_delta_accumulated(self):
        from app.assistant.framework.stream_executor import StreamingToolExecutor
        ...
        exec_ = StreamingToolExecutor(
            registry=PluginRegistry(), ctx=_mock_ctx(),
            max_concurrency=2,
        )
    ...
    def test_non_safe_tool_flushes_prior(self):
        ...
        safe = PluginDef(name="slow", description="s", func=slow_tool,
                       parameters=[], concurrency_safe=True)
        nonsafe = PluginDef(name="fast", description="f", func=fast_tool,
    ...
    def test_error_event_stops_stream(self):
        ...
        result = StreamingToolExecutor(
            registry=PluginRegistry(), ctx=_mock_ctx(), max_concurrency=1,
        ).run(iter(events))
    ...
    def test_tool_execution_exception_captured(self):
        ...
        bad_tool = PluginDef(name="bad", description="x", func=bad,
                           parameters=[], concurrency_safe=True)
    ...
    def test_streaming_autofills_name_for_load_skill(self):
        from app.assistant.framework.stream_executor import StreamingToolExecutor
        from app.assistant.framework.extension_registry import bootstrap_default_skills
        ...
        ctx = _mock_ctx()
        ctx.metadata["current_instruction"] = "HKSMM4 示例量度规则"
    ...
    def test_streaming_autofills_task_for_delegate_tools(self):
        ...
        ctx = _mock_ctx()
        ctx.metadata["current_instruction"] = "辅助组价"
        ...
        assert result.tool_calls[0]["arguments"] == {"task": "辅助组价"}
        assert result.tool_steps[0].tool_args == {"task": "辅助组价"}
    ...
    def test_streaming_autofills_query_for_semantic_tools(self):
        ...
        ctx = _mock_ctx()
        ctx.metadata["current_instruction"] = "HKSMM4 示例量度"
        ...
        assert result.tool_calls[0]["arguments"] == {"query": "HKSMM4 示例量度"}
        assert result.tool_steps[0].tool_args == {"query": "HKSMM4 示例量度"}
    ...
    def test_trace_records_tool_calls(self):
        ...
        tool = _make_stream_tool("t", '{"r":1}')
        reg = self._make_reg([tool])
        trace = LogCollector(handler_name="x", ctx=_mock_ctx())
        trace.start()
...
# ═════════════════════════════════════════════════════════════════
# T26: BaseHandler.stream_run() (H2)
# ═════════════════════════════════════════════════════════════════

class TestBaseHandlerStreamRun:
    """Tests for BaseHandler.stream_run() with mocked streaming provider."""

    def _make_stream_agent(self, tool_names):
        """Build a minimal streaming-enabled BaseHandler subclass."""
        from app.assistant.framework.base_handler import BaseHandler

        class _StreamHandler(BaseHandler):
            @property
    ...
    def test_simple_stream_answer(self):
        ...
        handler = self._make_stream_agent(["noop"])

        with patch("app.assistant.framework.base_handler.get_zh_provider", return_value=provider):
            result = handler.stream_run(_mock_ctx(), "hi", registry=reg)
    ...
    def test_stream_with_tool_call(self):
        ...
        provider = self._mock_provider(stream_events=stream_queue)
        handler = self._make_stream_agent(["search_x"])

        with patch("app.assistant.framework.base_handler.get_zh_provider", return_value=provider):
            result = handler.stream_run(_mock_ctx(), "search foo", registry=reg)
    ...
    def test_stream_error_propagates(self):
        ...
        provider = self._mock_provider(stream_events=stream_queue)
        handler = self._make_stream_agent([])

        with patch("app.assistant.framework.base_handler.get_zh_provider", return_value=provider):
            result = handler.stream_run(_mock_ctx(), "x", registry=reg)
...
class TestProviderStreamingInterface:
    """Tests that BaseZhProvider / DisabledZhProvider honor the streaming contract."""

    def test_disabled_provider_does_not_support_streaming(self):
        from app.assistant.providers.base import DisabledZhProvider

        p = DisabledZhProvider()
        assert p.supports_streaming() is False

    def test_base_provider_streaming_raises_by_default(self):
        from app.assistant.providers.base import BaseZhProvider

        # Build a minimal concrete subclass for testing the default behavior
        class _MinimalProvider(BaseZhProvider):
    ...
    def test_compatible_provider_supports_streaming(self):
        """The model-compatible provider declares streaming support."""
        ...
        p = ModelCompatProvider(settings)
        assert p.supports_streaming() is True

    def test_compatible_streaming_retries_without_stream_options_on_bad_request(self):
        from app.assistant.providers.model_compat import ModelCompatProvider
    ...
    def test_compatible_streaming_bad_request_falls_back_to_sync_tools(self):
        from app.assistant.providers.model_compat import ModelCompatProvider
    ...
    def test_compatible_tool_calling_retries_without_tool_choice_on_bad_request(self):
        from app.assistant.providers.model_compat import ModelCompatProvider
...
# ═════════════════════════════════════════════════════════════════
# T28: ContextStore — InMemoryContextStore (H3)
# ═════════════════════════════════════════════════════════════════

class TestInMemoryContextStore:
    """Tests for the in-memory reference implementation of ContextStore."""

    def _store(self):
        from app.assistant.framework.context_store import InMemoryContextStore
    ...
    def test_invalid_scope_raises(self):
        from app.assistant.framework.context_store import MemoryValidationError
        s = self._store()
...
class TestMemoryTools:
    ...
    def _ctx_with_store(self):
        ...
        ctx.memory = InMemoryContextStore()
        return ctx
...
# ═════════════════════════════════════════════════════════════════
# T30: BaseHandler memory context injection (H3)
# ═════════════════════════════════════════════════════════════════

class TestMemoryContextInjection:
    """Tests for BaseHandler.build_memory_context() and use_memory_context flag."""

    def _agent(self, *, use_memory=False, limit=5):
        from app.assistant.framework.base_handler import BaseHandler

        class _A(BaseHandler):
            @property
    ...
    def test_default_no_injection(self):
        ...
        handler = self._agent(use_memory=False)
        msg = handler.build_user_message(ctx, "hello")
        assert msg == "hello"
    ...
    def test_injects_when_enabled(self):
        ...
        handler = self._agent(use_memory=True)
        msg = handler.build_user_message(ctx, "请组价")
    ...
    def test_no_context_store_returns_instruction(self):
        ctx = _mock_ctx(user_id=7, project_id=42)
        ctx.memory = None
        handler = self._agent(use_memory=True)
        msg = handler.build_user_message(ctx, "do x")
        assert msg == "do x"
    ...
    def test_limit_respected(self):
        ...
        handler = self._agent(use_memory=True, limit=2)
        msg = handler.build_user_message(ctx, "do x")
...
# ═════════════════════════════════════════════════════════════════
# T31: ConfigurableHandler memory integration via YAML (H3)
# ═════════════════════════════════════════════════════════════════

class TestConfigurableHandlerMemory:
    """Tests that ConfigurableHandler honors memory fields from YAML."""
...
body"""
    ...
    def test_configurable_handler_mirrors_definition(self):
        from app.assistant.framework.handler_definition import HandlerDefinition
        from app.assistant.framework.configurable_handler import ConfigurableHandler
        ...
        handler = ConfigurableHandler(defn)
        assert handler.use_memory_context is True
        assert handler.memory_context_limit == 7

    def test_configurable_handler_injects_memory(self):
        from app.assistant.framework.handler_definition import HandlerDefinition
        from app.assistant.framework.configurable_handler import ConfigurableHandler
        from app.assistant.framework.context_store import InMemoryContextStore
        ...
        with patch.object(ctx, 'build_project_context', return_value=''):
            msg = handler.build_user_message(ctx, "do task")
        assert "2018 定额" in msg
        assert "do task" in msg


# ═════════════════════════════════════════════════════════════════
# T32: Extension dataclass + parser (H4)
# ═════════════════════════════════════════════════════════════════
...
class TestExtensionParser:
    """Tests for Extension dataclass + parse_extension_text."""

    def test_parse_valid_skill(self):
        from app.assistant.framework.skill import parse_extension_text

        s = parse_extension_text(_SAMPLE_SKILL_TEXT)
        assert s.name == "sample_skill"
        assert s.title == "样例技能"
        assert s.description == "样例技能"
    ...
    def test_missing_frontmatter_raises(self):
        from app.assistant.framework.skill import ExtensionParseError, parse_extension_text

        with pytest.raises(ExtensionParseError):
            parse_extension_text("no frontmatter here")
...
"""
        with pytest.raises(ExtensionParseError):
            parse_extension_text(empty_body)
...
class TestExtensionFileLoad:
    """Tests for parse_extension_file + directory loading of real shipped skills."""

    def test_load_shipped_skills(self):
        from app.assistant.framework.extension_registry import load_skills_from_dir
        skills_dir = Path(__file__).parent.parent / "app" / "zh" / "skills"
...
# ═════════════════════════════════════════════════════════════════
# T33: ExtensionRegistry (H4)
# ═════════════════════════════════════════════════════════════════

class TestExtensionRegistry:
    """Tests for ExtensionRegistry behavior."""

    def _fresh_registry(self):
        from app.assistant.framework.extension_registry import ExtensionRegistry
        return ExtensionRegistry()

    def _mk_skill(self, name, *, triggers=None, tags=None, applies_to=None):
        from app.assistant.framework.skill import Extension
        return Extension(
            name=name,
    ...
    def test_match_by_query(self):
        r = self._fresh_registry()
        r.register(self._mk_skill("hk", triggers=["HKSMM4", "示例区"]))
    ...
    def test_register_empty_name_raises(self):
        ...
        r = self._fresh_registry()
        bad = Extension(name="", title="t", description="d", body="b")
        with pytest.raises(ValueError):
            r.register(bad)


class TestExtensionBootstrap:
    ...
    def test_bootstrap_populates_registry(self):
        ...
        names = extension_registry.all_names()
        assert "hksmm4_basics" in names
        assert "gb50500_compliance" in names
        assert "concrete_pricing_tips" in names
    ...
    def test_bootstrap_is_idempotent(self):
        ...
        bootstrap_default_skills(force=True)
        count_before = len(extension_registry)
        # Second call without force should not duplicate
        added = bootstrap_default_skills()
        assert added == 0
        assert len(extension_registry) == count_before


# ═════════════════════════════════════════════════════════════════
# T34: ConfigurableHandler skill injection (H4)
# ═════════════════════════════════════════════════════════════════

class TestConfigurableHandlerExtensions:
    """Tests that ConfigurableHandler injects declared skills into system_prompt."""
    ...
    def test_no_skills_returns_base_prompt(self):
        ...
        handler = ConfigurableHandler(defn)
        assert handler.system_prompt == "BASE_PROMPT"
    ...
    def test_extension_names_accessor(self):
        from app.assistant.framework.handler_definition import HandlerDefinition
        from app.assistant.framework.configurable_handler import ConfigurableHandler
        ...
        handler = ConfigurableHandler(defn)
        assert handler.extension_names == ["a", "b"]
...
class TestExtensionTools:
    ...
    def test_load_extension_tool_returns_body(self):
        import app.assistant.tools  # noqa: F401
        from app.assistant.framework.plugin_registry import registry
    ...
    def test_load_extension_missing_returns_error(self):
        import app.assistant.tools  # noqa: F401
        from app.assistant.framework.plugin_registry import registry
    ...
    def test_extension_tools_are_read_only_and_concurrency_safe(self):
        import app.assistant.tools  # noqa: F401
        from app.assistant.framework.plugin_registry import registry
...
# ═════════════════════════════════════════════════════════════════
# T36: EmbedProvider + vector_utils (H5)
# ═════════════════════════════════════════════════════════════════

class TestHashEmbedProvider:
    """Tests for HashEmbedProvider — the deterministic offline embedder."""

    def test_dim_configurable(self):
        from app.assistant.framework.embed_provider import HashEmbedProvider
        p = HashEmbedProvider(dim=64)
        assert p.dim == 64
        assert len(p.embed("hello")) == 64

    def test_deterministic(self):
        from app.assistant.framework.embed_provider import HashEmbedProvider
        p = HashEmbedProvider(dim=128)
    ...
    def test_normalized_to_unit_length(self):
        from app.assistant.framework.embed_provider import HashEmbedProvider
        from app.assistant.framework.vector_utils import l2_norm
        p = HashEmbedProvider()
    ...
    def test_cjk_vs_ascii_distinct(self):
        from app.assistant.framework.embed_provider import HashEmbedProvider
        from app.assistant.framework.vector_utils import dot
        p = HashEmbedProvider(dim=256)
        v_cjk = p.embed("混凝土")
    ...
    def test_dim_minimum(self):
        from app.assistant.framework.embed_provider import HashEmbedProvider
        with pytest.raises(ValueError):
            HashEmbedProvider(dim=4)


class TestEmbedProviderSingleton:
    """Tests for get_embed_provider / set_embed_provider."""

    def test_default_fallback_without_api_key(self, monkeypatch):
        from app.assistant.framework.embed_provider import (
            HashEmbedProvider,
            get_embed_provider,
            reset_embed_provider,
        )
        monkeypatch.delenv("MODEL_COMPAT_API_KEY", raising=False)
    ...
    def test_force_hash_backend(self, monkeypatch):
        ...
        monkeypatch.setenv("MODEL_COMPAT_API_KEY", "sk-fake")   # should still be ignored
        reset_embed_provider()
        p = get_embed_provider()
        assert isinstance(p, HashEmbedProvider)

    def test_set_embed_provider(self):
        from app.assistant.framework.embed_provider import (
            HashEmbedProvider,
            get_embed_provider,
            reset_embed_provider,
        ...
        custom = HashEmbedProvider(dim=32)
        set_embed_provider(custom)
        assert get_embed_provider() is custom
        # Cleanup
        reset_embed_provider()
...
class TestVectorUtils:
    ...
    def test_cosine_identical_unit_vectors(self):
        from app.assistant.framework.vector_utils import cosine_similarity
    ...
    def test_normalize(self):
        from app.assistant.framework.vector_utils import l2_norm, normalize
        v = [3.0, 4.0]  # norm = 5
        nv = normalize(v)
    ...
    def test_top_k_respects_k(self):
        from app.assistant.framework.vector_utils import top_k
...
# ═════════════════════════════════════════════════════════════════
# T37: ContextStore.search_semantic (H5)
# ═════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════
# T38: ExtensionRegistry.match_semantic (H5)
# ═════════════════════════════════════════════════════════════════

class TestExtensionMatchSemantic:
...
# ═════════════════════════════════════════════════════════════════
# T40: Orchestrator integrates Memory + Extensions (H6)
# ═════════════════════════════════════════════════════════════════

class TestOrchestratorMemoryExtensions:
    """Tests for Phase H6: Orchestrator auto-injects memory + skill hints
    and exposes memory/skill management tools in its registry."""

    def _orch(self):
        import app.assistant.tools  # noqa: F401 — ensure tools are registered
        from app.assistant.agents.v2.orchestrator import OrchestratorHandler
        from app.assistant.framework.extension_registry import bootstrap_default_skills
        bootstrap_default_skills()
    ...
    def test_build_user_message_injects_memory(self):
        ...
        ctx.memory = InMemoryContextStore()
        ctx.memory.save(scope="project", scope_id=42, key="basis",
                        content="本项目按示例省2018定额计价", importance=5)
        ...
        assert "历史记忆" in msg
        assert "本项目按示例省2018定额计价" in msg
        assert "用户偏好 m² 展开" in msg
        assert "为未绑定项组价" in msg

    def test_build_user_message_injects_extension_hints(self):
        ...
        msg = orch.build_user_message(
            ctx, "请参考 HKSMM4 示例量度规范帮我审核这个项目")

        # Extension hints block should appear
        assert "可能相关的领域知识" in msg or "Extensions" in msg
    ...
    def test_build_user_message_extension_hints_respect_limit(self):
        """extension_prefetch_limit controls number of hinted skills."""
        orch = self._orch()
        orch.extension_prefetch_limit = 1  # tighten
        # Lower threshold to force at least one match
        orch.extension_prefetch_min_similarity = 0.0
        ...
        from app.assistant.framework.extension_registry import extension_registry
        hinted_count = sum(
            1 for s in extension_registry.all_skills() if f"**{s.name}**" in msg
        )
    ...
    def test_build_user_message_no_skills_match(self):
        ...
        orch = self._orch()
        orch.extension_prefetch_min_similarity = 0.9  # very strict
        ctx = _mock_ctx(user_id=7, project_id=42)
        ctx.memory = None
    ...
    def test_build_user_message_no_context_store_tolerated(self):
    ...
    def test_system_prompt_documents_memory_skills(self):
        """System prompt now explicitly references Memory + Extensions workflow."""
        orch = self._orch()
        prompt = orch.system_prompt
        assert "save_memory" in prompt
        assert "load_skill" in prompt
        assert "Memory" in prompt or "记忆" in prompt
        assert "Extensions" in prompt or "领域知识" in prompt
    ...
    def test_build_user_message_combines_memory_skills_and_task(self):
        ...
        msg = orch.build_user_message(ctx, "HKSMM4 示例工程量审核")
...
# ═════════════════════════════════════════════════════════════════
# T41: Auto-sediment memories (H7) — ContextExtractor + Orchestrator.on_result
# ═════════════════════════════════════════════════════════════════
...
class TestExtractedMemoryDTO:
    ...
    def test_valid(self):
        from app.assistant.framework.context_extractor import ExtractedMemory
...
class TestLLMContextExtractor:
    ...
    def test_invalid_max_items_raises(self):
        ...
        with pytest.raises(ValueError):
            LLMContextExtractor(max_items=0)

    def test_empty_input_returns_empty(self):
        from app.assistant.framework.context_extractor import LLMContextExtractor
        e = LLMContextExtractor()
        ctx = _mock_ctx(user_id=1, project_id=1)
    ...
    def test_parse_clean_json(self):
        ...
        e = LLMContextExtractor(provider=self._stub_provider(response))
        ctx = _mock_ctx(user_id=1, project_id=1)
    ...
    def test_parse_invalid_json_returns_empty(self):
        from app.assistant.framework.context_extractor import LLMContextExtractor
        e = LLMContextExtractor(provider=self._stub_provider("not json at all"))
    ...
    def test_provider_disabled_returns_empty(self):
        from app.assistant.framework.context_extractor import LLMContextExtractor
        e = LLMContextExtractor(provider=self._stub_provider(
    ...
    def test_provider_exception_returns_empty(self):
        ...
        e = LLMContextExtractor(provider=p)
        assert e.extract("i", "a", _mock_ctx()) == []
...
class TestOrchestratorAutoSaveMemory:
    ...
    def _setup(self, candidates):
        ...
        import app.assistant.tools  # noqa: F401
        from app.assistant.agents.v2.orchestrator import OrchestratorHandler
        from app.assistant.framework.context_extractor import ContextExtractor
        from app.assistant.framework.context_store import InMemoryContextStore
        from app.assistant.framework.extension_registry import bootstrap_default_skills
        from app.assistant.framework.types import HandlerResult
        ...
        orch.context_extractor = _StubExtractor()
        ...
        orch._last_ctx = ctx
        result = HandlerResult(answer="a detailed answer", error=None)
    ...
    def test_default_extractor_is_noop(self):
        """Without setting context_extractor, the default Noop returns nothing."""
        import app.assistant.tools  # noqa: F401
        from app.assistant.agents.v2.orchestrator import OrchestratorHandler
        from app.assistant.framework.context_store import InMemoryContextStore
        from app.assistant.framework.types import HandlerResult
        ...
        orch = OrchestratorHandler()
        orch.auto_save_memory = True
        # Note: context_extractor is NOT set → lazy default kicks in
        ctx = _mock_ctx(user_id=7, project_id=42)
        ctx.memory = InMemoryContextStore()
        orch._last_instruction = "i"
        orch.on_result(ctx, HandlerResult(answer="a"))
    ...
    def test_extractor_exception_swallowed(self):
        ...
        class _Boom(ContextExtractor):
            def extract(self, instruction, answer, ctx):
                raise RuntimeError("boom")

        orch = OrchestratorHandler()
        orch.auto_save_memory = True
        orch.context_extractor = _Boom()

        ctx = _mock_ctx(user_id=7, project_id=42)
        ctx.memory = InMemoryContextStore()
        orch._last_instruction = "i"
        result = HandlerResult(answer="a")
        out = orch.on_result(ctx, result)
        assert out is result

    def test_full_end_to_end_with_llm_extractor(self):
        """Use LLMContextExtractor with a stubbed provider to hit the full pipeline."""
        from app.assistant.agents.v2.orchestrator import OrchestratorHandler
        from app.assistant.framework.context_extractor import LLMContextExtractor
        from app.assistant.framework.context_store import InMemoryContextStore
        from app.assistant.framework.types import HandlerResult
        ...
        response = json.dumps({
            "memories": [
                {"scope": "project", "key": "basis",
                 "content": "本项目按示例省2018定额", "importance": 5,
                 "tags": ["basis"]},
            ]
        })
        p = MagicMock()
        ...
        orch = OrchestratorHandler()
        orch.auto_save_memory = True
        orch.context_extractor = LLMContextExtractor(provider=p)

        ctx = _mock_ctx(user_id=7, project_id=42)
        ctx.memory = InMemoryContextStore()
        orch._last_instruction = "为项目设定计价依据"
        orch.on_result(ctx, HandlerResult(
            answer="已确认本项目按示例省2018定额进行计价。"
        ))

        proj = ctx.memory.list(scope="project", scope_id=42)
        assert len(proj) == 1
        assert proj[0].key == "basis"
        assert "示例省2018定额" in proj[0].content
...
class TestMemorySettingsFromEnv:
    ...
    def test_defaults(self, monkeypatch):
        from app.assistant.config import get_memory_settings
        for k in ("EMBEDDING_BACKEND", "ZH_AUTO_SAVE_MEMORY",
                  "ZH_MEMORY_EXTRACTOR_MAX_ITEMS"):
            monkeypatch.delenv(k, raising=False)
        s = get_memory_settings()
        assert s.embedding_backend == ""
        assert s.auto_save_memory_default is False
        assert s.context_extractor_max_items == 3
    ...
    def test_auto_save_truthy(self, monkeypatch):
        ...
        for val in ("true", "1", "yes", "on"):
            monkeypatch.setenv("ZH_AUTO_SAVE_MEMORY", val)
    ...
    def test_max_items_custom(self, monkeypatch):
        from app.assistant.config import get_memory_settings
        monkeypatch.setenv("ZH_MEMORY_EXTRACTOR_MAX_ITEMS", "5")
        assert get_memory_settings().context_extractor_max_items == 5

    def test_max_items_invalid_uses_default(self, monkeypatch):
        from app.assistant.config import get_memory_settings
        monkeypatch.setenv("ZH_MEMORY_EXTRACTOR_MAX_ITEMS", "abc")
        assert get_memory_settings().context_extractor_max_items == 3

    def test_max_items_floor(self, monkeypatch):
        from app.assistant.config import get_memory_settings
        monkeypatch.setenv("ZH_MEMORY_EXTRACTOR_MAX_ITEMS", "0")
        assert get_memory_settings().context_extractor_max_items == 1
...
class TestResolveAutoSave:
    ...
    def test_request_override_takes_precedence(self, monkeypatch):
        ...
        monkeypatch.setenv("ZH_AUTO_SAVE_MEMORY", "true")
    ...
    def test_none_falls_back_to_env(self, monkeypatch):
        ...
        monkeypatch.setenv("ZH_AUTO_SAVE_MEMORY", "false")
...
class TestBuildProductionOrchestrator:
    ...
    def test_auto_save_off(self):
        ...
        handler = _build_production_orchestrator(auto_save=False)
        assert handler.auto_save_memory is False

    def test_auto_save_on_installs_llm_extractor(self, monkeypatch):
        monkeypatch.setenv("ZH_MEMORY_EXTRACTOR_MAX_ITEMS", "2")
        from app.assistant.framework.context_extractor import LLMContextExtractor
        from app.api.routes.orchestrator import _build_production_orchestrator

        handler = _build_production_orchestrator(auto_save=True)
        assert handler.auto_save_memory is True
        assert isinstance(agent.context_extractor, LLMContextExtractor)
        # max_items propagated from env
        assert handler.context_extractor._max_items == 2  # noqa: SLF001
    ...
    def test_ctx_with_memory_has_sqla_store(self, db):
        ...
        assert isinstance(ctx.memory, SQLAlchemyContextStore)
...
class TestOrchestrateRouteIntegration:
    ...
    With ZH_PROVIDER=disabled (test default), BaseHandler.run() short-circuits
    with `zh_not_configured`, which is perfect for verifying the route wiring
    ...
    def test_orchestrate_request_shape(self, client, db, monkeypatch):
        ...
        monkeypatch.setenv("ZH_PROVIDER", "disabled")
    ...
    def test_orchestrate_returns_error_when_zh_disabled(self, client, db, monkeypatch):
        """With 辅助 disabled, orchestrate should short-circuit gracefully."""
        monkeypatch.setenv("ZH_PROVIDER", "disabled")
        pid = self._create_project(db)
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert data["error"] == "zh_not_configured"

    def test_orchestrate_persists_pre_seeded_memory(self, client, db, monkeypatch):
        """Prove the SQLAlchemyContextStore is actually wired into ctx.

        Seed a memory via the store directly; the route then runs the
        orchestrator (which gracefully fails due to ZH_PROVIDER=disabled),
        ...
        from app.assistant.framework.context_store import SQLAlchemyContextStore
        store = SQLAlchemyContextStore(db)
        store.save(
...
# ═════════════════════════════════════════════════════════════════
# T44: Extensions browsing REST API (H9)
# ═════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════
# T45: End-to-end Orchestrator tests with StubZhProvider
# ═════════════════════════════════════════════════════════════════


class _StubZhProvider:
    """A programmable 辅助 provider for E2E testing.
    ...
    When ``tool_calls`` is present in a response, the handler loop will
    ...
    so tests can inspect what the handler saw.
    """
    ...
    def generate_with_tools(self, *, task, messages, tools, model=None):
        # model 参数由 base_handler 传入（OPT-4），stub 忽略该参数以保持兼容
    ...
    def generate_text(self, *, task, messages, model=None):
        """Used by LLMContextExtractor."""
        self.captured_messages.append([dict(m) for m in messages])
        if self._turn >= len(self._responses):
            return '{"memories": []}'
        resp = self._responses[self._turn]
    ...
    def generate_structured(self, *, task, messages, schema_model, model=None):
...
class _StubExtractor:
    """A deterministic ContextExtractor for E2E testing.
...
class TestE2EOrchestratorWithStub:
    """End-to-end tests that exercise the real OrchestratorHandler with a
    StubZhProvider, verifying memory injection, skill hints, tool
    ...
    def _make_ctx(db, project_id=1, user_id=1):
        from app.assistant.framework.context import HandlerContext
        from app.assistant.framework.context_store import SQLAlchemyContextStore
        ctx = HandlerContext(db=db, project_id=project_id, user_id=user_id)
        ctx.memory = SQLAlchemyContextStore(db)
        return ctx
    ...
    def _make_orchestrator(*, auto_save=False, extractor=None):
        ...
        if extractor is not None:
            orch.context_extractor = extractor
        return orch

    # ── Test 1: Memory block + skill hints injected into the user message ──

    def test_memory_and_extension_hints_in_prompt(self, db, monkeypatch):
        ...
        stub = _StubZhProvider(responses=[
            {"content": "已了解项目背景，准备协调子Handler执行。"},
        ])

        monkeypatch.setattr(
            "app.assistant.framework.base_handler.get_zh_provider", lambda: stub,
        )

        orch = self._make_orchestrator()
        result = orch.run(ctx, "查看项目概况")

        assert result.answer == "已了解项目背景，准备协调子Handler执行。"
        assert result.error is None
        ...
        # Extension hints are best-effort with hash embedder; the dedicated
        # test_extension_hints_structure covers that path with a better query.
    ...
    def test_auto_save_memory_full_chain(self, db, monkeypatch):
        ...
        stub_provider = _StubZhProvider(responses=[
            {"content": "本项目按示例省2018定额计价，混凝土采用C30。"},
        ])
        monkeypatch.setattr(
        ...
        assert result.error is None
        assert "示例省2018" in result.answer
        ...
        assert "示例省2018" in mem1.content
    ...
    def test_multi_turn_tool_call_loop(self, db, monkeypatch):
        """Stub returns a tool call on turn 1 → handler executes it →
    ...
    # ── Test 6: Auto-save with LLMContextExtractor (stub provider for both) ──

    def test_auto_save_with_llm_extractor_stub(self, db, monkeypatch):
        """Wire a real LLMContextExtractor but with a stub provider that
        ...
        # 2. By LLMContextExtractor → generate_text (extraction JSON)
        ...
        stub = _StubZhProvider(responses=[
            ...
            # Turn 2: LLMContextExtractor's generate_text() call
        ...
        # Wire LLMContextExtractor with the same stub as provider override
        extractor = LLMContextExtractor(max_items=3, provider=stub)
        orch = self._make_orchestrator(
    ...
    # ── Test 7: Extension hint block structure ──

    def test_extension_hints_structure(self, db, monkeypatch):
        ...
        user_msg = stub.captured_messages[0][1]["content"]
        assert "## 可能相关的领域知识（Extensions）" in user_msg
...
# ═════════════════════════════════════════════════════════════════
# T-NEW-1: Phase I Handler Integrity (ProjectSetup + FullPipeline)
# ═════════════════════════════════════════════════════════════════

class TestPhaseIHandlers:
    """Tests for ProjectSetupHandler and FullPipelineHandler."""

    def test_project_setup_handler_config(self):
        from app.assistant.agents.v2.project_setup_handler import ProjectSetupHandler
        handler = ProjectSetupHandler()
        assert handler.name == "project_setup"
        assert handler.max_turns == 25
        assert "batch_create_boq_items" in handler.tool_names
        assert "update_boq_item" in handler.tool_names
        assert "delete_boq_items" in handler.tool_names
        assert "search_standard_codes" in handler.tool_names
        assert len(agent.tool_names) == 8

    def test_full_pipeline_handler_config(self):
        from app.assistant.agents.v2.full_pipeline_handler import FullPipelineHandler
        handler = FullPipelineHandler()
        assert handler.name == "full_pipeline"
        assert handler.max_turns == 80
        assert "batch_bind_quotas" in handler.tool_names
        assert "auto_match_and_bind" in handler.tool_names
        assert "batch_auto_match_all" in handler.tool_names
        assert "batch_calculate_project" in handler.tool_names
        assert "recalculate_dirty" in handler.tool_names
        assert "update_boq_item" in handler.tool_names
        assert "delete_boq_items" in handler.tool_names
        assert len(agent.tool_names) == 21

    def test_phase_i_handler_tools_all_exist(self):
        import app.assistant.tools
        from app.assistant.framework.plugin_registry import registry
        from app.assistant.agents.v2.project_setup_handler import ProjectSetupHandler
        from app.assistant.agents.v2.full_pipeline_handler import FullPipelineHandler
        ...
        all_tools = set(registry.all_names)
        for handler in [ProjectSetupHandler(), FullPipelineHandler()]:
            missing = set(agent.tool_names) - all_tools
            assert missing == set(), f"Phase I handler '{handler.name}' references missing tools: {missing}"
    ...
    def test_project_setup_build_user_message_basic(self):
        ...
        msg = handler.build_user_message(ctx, "建一个5层住宅")
    ...
    def test_full_pipeline_build_user_message_injects_state(self):
        ...
        msg = handler.build_user_message(ctx, "一键全流程")
        assert "全流程" in msg


# ═════════════════════════════════════════════════════════════════
# T-NEW-2: CostExecuteHandler Batch Tool Integration
# ═════════════════════════════════════════════════════════════════

class TestCostExecuteHandlerBatch:
    """Tests for CostExecuteHandler with new batch tools."""
    ...
    def test_has_batch_tools(self):
        ...
        assert "batch_calculate_project" in handler.tool_names
        assert "recalculate_dirty" in handler.tool_names
        assert "list_unbound_items" in handler.tool_names
        assert "update_boq_item" in handler.tool_names
...
class TestOrchestratorRouting:
    ...
    def test_project_setup_routing(self):
        from app.assistant.agents.v2.orchestrator import OrchestratorHandler
        orch = OrchestratorHandler()
        directive = orch._execution_directive("辅助开项：5层住宅")
