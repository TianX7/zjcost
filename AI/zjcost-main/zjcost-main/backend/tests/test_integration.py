...
def _seed_boq_and_quota(db, project_id):
    ...
    db.add_all([
        MaterialPrice(code="INT-L", name="人工费", unit="工日", unit_price=100.0),
        MaterialPrice(code="INT-M", name="材料费", unit="t", unit_price=50.0),
        MaterialPrice(code="INT-N", name="机械费", unit="台班", unit_price=80.0),
    ])
    db.commit()
    db.refresh(boq)
    db.refresh(quota)
