...
def _seed(db, project_id):
    ...
    db.add_all([boq1, q1])
    # 注入材料价格数据，避免 _lookup_price 返回 None 导致 direct_cost 为 0
    db.add_all([
        MaterialPrice(code="SP4-L", name="人工费", unit="工日", unit_price=100.0),
        MaterialPrice(code="SP4-M", name="材料费", unit="t", unit_price=50.0),
        MaterialPrice(code="SP4-N", name="机械费", unit="台班", unit_price=80.0),
    ])
    db.commit()
    db.refresh(boq1)
    db.refresh(q1)
...
def test_create_rule_package(client):
    r = client.post("/api/rule-packages", json={
        "name": "示例2024", "region": "demo-b",
        "management_rate": 0.10, "profit_rate": 0.06,
        "regulatory_rate": 0.04, "tax_rate": 0.09,
    })
    assert r.status_code == 200
    rp = r.json()
    assert rp["name"] == "示例2024"
    assert rp["management_rate"] == 0.10


def test_list_rule_packages(client):
    client.post("/api/rule-packages", json={"name": "A", "region": "demo-a"})
    client.post("/api/rule-packages", json={"name": "B", "region": "demo-b"})
...
def test_bind_rule_package(client):
    # Create project
    r = client.post("/api/projects", json={"name": "RuleBind", "region": "demo-a"})
    pid = r.json()["id"]
    # Create rule package
    r = client.post("/api/rule-packages", json={"name": "BJ2024", "region": "demo-a"})
    rp_id = r.json()["id"]
    # Bind
...
def test_calculate_with_rule_package(client, db):
    ...
    r = client.post("/api/projects", json={"name": "CalcRP", "region": "demo-b"})
    pid = r.json()["id"]
    boq1, q1 = _seed(db, pid)

    # Create a rule package with different rates
    r = client.post("/api/rule-packages", json={
        "name": "高费率", "region": "demo-b",
    ...
    r2 = client.post("/api/projects", json={"name": "CalcDef", "region": "demo-b"})
    pid2 = r2.json()["id"]
...
def test_calculate_with_material_prices(client, db):
    ...
    r = client.post("/api/projects", json={"name": "CalcMP", "region": "demo-b"})
    pid = r.json()["id"]
    _seed(db, pid)
...
def test_export_diff_report(client, db):
    r = client.post("/api/projects", json={"name": "DiffExp", "region": "demo-a"})
    pid = r.json()["id"]
    _seed(db, pid)
...
def test_audit_log_on_bind(client):
    ...
    r = client.post("/api/projects", json={"name": "AuditP", "region": "demo-a"})
    pid = r.json()["id"]
    r = client.post("/api/rule-packages", json={"name": "AuditRP", "region": "demo-a"})
    rp_id = r.json()["id"]
...
def test_audit_log_empty(client):
    r = client.post("/api/projects", json={"name": "NoLog", "region": "demo-b"})
    pid = r.json()["id"]
    r = client.get(f"/api/projects/{pid}/audit-logs")
    assert r.status_code == 200
    assert r.json() == []
